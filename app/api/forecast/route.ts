...
const safeDays=(value:unknown)=>{const n=Number(value);return [30,60,90,180,365].includes(n)?n:90;};
...
export async function GET(request:NextRequest){
  ...
  const {data,error}=await supabase.rpc("financial_app_forecast_overview",{p_start:madridToday(),p_days:safeDays(request.nextUrl.searchParams.get("days"))});
...
export async function POST(request:NextRequest){
  ...
  const {data,error}=await supabase.rpc("financial_app_upsert_forecast",{
    ...
    p_title:title,p_date:date,p_amount:amount,
    ...
    p_confidence:Number.isFinite(Number(body.confidence))?Number(body.confidence):1,
