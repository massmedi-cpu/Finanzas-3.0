...
function apiError(error: unknown) {
  if (error instanceof ConfigurationRequestError) {
    return Response.json({ error: "invalid_request", code: error.code, message: error.message }, { status: 400 });
  }
...
export async function GET() {
  try {
    ...
    const [accounts, categories] = await Promise.all([service.listAccounts(), service.listCategories()]);
    return Response.json(
      { accounts, categories },
...
export async function POST(request: Request) {
  try {
    ...
    switch (command.operation) {
      case "account.create":
        return Response.json({ account: await service.createAccount(command.draft) }, { status: 201 });
      case "account.update":
        return Response.json({ account: await service.updateAccount(command.id, command.draft) });
      case "account.archive":
        return Response.json({ account: await service.setAccountArchived(command.id, command.archived) });
      case "account.reorder":
        await service.reorderAccounts(command.orderedIds);
        return Response.json({ ok: true });
      case "category.create":
        return Response.json({ category: await service.createCategory(command.draft) }, { status: 201 });
      case "category.update":
        return Response.json({ category: await service.updateCategory(command.id, command.draft) });
      case "category.archive":
        return Response.json({ category: await service.setCategoryArchived(command.id, command.archived) });
      case "category.reorder":
        await service.reorderCategories(command.orderedIds);
        return Response.json({ ok: true });
      case "category.impact": {
        const result = await callPersistenceGateway<{ impact: CategoryImpact }>("category.impact", { id: command.id });
