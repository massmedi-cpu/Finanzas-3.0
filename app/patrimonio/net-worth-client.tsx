...
export function NetWorthClient({ initial }: { initial: NetWorthOverview }) {
  ...
  return <>
    {error && <div className="inline-alert error nw-error" role="alert">{error}</div>}
    ...
    {editor && <div className="drawer-backdrop nw-editor-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setEditor(null); }}><section className="net-worth-drawer nw-editor" role="dialog" aria-modal="true" aria-labelledby="nw-editor-title" aria-busy={busy ? "true" : undefined}><div className="drawer-head nw-editor-head"><div><p className="eyebrow">PATRIMONIO MANUAL</p><h2 id="nw-editor-title">{editor.id ? "Editar elemento" : "Añadir elemento"}</h2></div><button className="icon-button" aria-label="Cerrar" disabled={busy} onClick={() => setEditor(null)}>×</button></div><div className="nw-form">
      ...
      <div className="nw-editor-actions"><button className="ghost" disabled={busy} onClick={() => setEditor(null)}>Cancelar</button><button className="primary-action" disabled={busy} aria-busy={busy ? "true" : undefined} onClick={() => void save()}>{busy ? "Guardando…" : "Guardar"}</button></div>
    </div></section></div>}
  </>;
}
