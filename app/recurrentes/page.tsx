  ...
  try {
    ...
    rows = patterns.map((pattern) => {
      ...
      return {
        ...
        expectedAmount: customAmount !== null && Number.isFinite(customAmount) ? customAmount : pattern.averageAmount,
        intervalDays: pattern.intervalDays,
        occurrences: pattern.occurrences,
        lastDate: pattern.lastDate,
        nextDate: preference?.next_expected_date || automaticNext,
        confidence: pattern.confidence,
        status: preference?.status || 'auto',
        displayName: preference?.display_name || pattern.description,
        notes: preference?.notes || '',
        customized: Boolean(preference),
      };
    });
  } catch {
    sourceError = true;
  ...
  return (
    <main className="page">
      <section className="page-header">
        <div>
          <div className="eyebrow">Recurrentes</div>
          <h1>Controla lo que se repite cada mes</h1>
          <p className="subtitle">Confirma recibos, suscripciones e ingresos habituales, corrige importe o próxima fecha y decide qué patrones deben entrar en tus previsiones.</p>
        </div>
        {rows.length > 0 && <span className="badge">{rows.length} patrones detectados</span>}
      </section>

      {sourceError ? (
        <div className="status-panel status-danger">
          <div>
            <div className="status-title">No se han podido analizar los recurrentes</div>
            <div className="status-copy">No se muestran patrones parciales mientras la fuente bancaria no pueda validarse.</div>
          </div>
        </div>
      ) : rows.length === 0 ? (
        <section className="card"><div className="empty">Todavía no hay patrones mensuales con suficiente histórico para tratarlos como recurrentes.</div></section>
      ) : (
        <RecurringManager initialRows={rows} categories={categories} />
      )}
    </main>
  );
}
