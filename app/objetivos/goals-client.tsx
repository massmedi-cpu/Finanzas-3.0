...
export function GoalsClient({initialData}:{initialData:GoalsOverview}){
  ...
  return <div className={`goals-module ${loading?"is-loading":""}`}>
    ...
    <section className="goals-summary" aria-label="Resumen de objetivos">
      <article><span>Objetivo total</span><strong>{data.summary.activeCount?money.format(data.summary.targetTotal):"—"}</strong><small>{data.summary.activeCount?`${knownProgress.toLocaleString("es-ES",{maximumFractionDigits:1})}% registrado`:"Crea tu primera meta"}</small></article>
      <article><span>Progreso conocido</span><strong>{data.summary.activeCount?money.format(data.summary.trackedTotal):"—"}</strong><small>Solo importes manuales o saldos reales</small></article>
      <article><span>Por completar</span><strong>{data.summary.activeCount?money.format(data.summary.remainingTotal):"—"}</strong><small>{data.summary.achievedCount} conseguidos</small></article>
      <article className={data.summary.attentionCount||data.summary.overdueCount?"warning":"good"}><span>Aportación mensual</span><strong>{data.summary.monthlyRequired>0?money.format(data.summary.monthlyRequired):"—"}</strong><small>{data.summary.attentionCount} en atención · {data.summary.overdueCount} vencidos</small></article>
      <article><span>Capacidad de referencia</span><strong>{money.format(data.capacityReference)}</strong><small>Media de cash flow neto de 3 meses cerrados</small></article>
    </section>
    ...
    <section className="goals-panel"><div className="goals-panel-head"><div><p className="eyebrow">PLAN PERSONAL</p><h2>Metas y progreso</h2></div><span className="pill">{data.summary.sourceMissingCount?`${data.summary.sourceMissingCount} sin saldo real`:"Fuentes verificadas"}</span></div>
      ...
      <div className="goals-list">{data.goals.map(goal=>{
        ...
        return <article className={`goal-card status-${goal.status}`} key={goal.id}>
          ...
          <div className="goal-amounts"><div><span>Progreso</span><strong>{goal.progressAmount==null?"—":money.format(goal.progressAmount)}</strong></div><div><span>Objetivo</span><strong>{money.format(goal.targetAmount)}</strong></div><div><span>Falta</span><strong>{goal.remainingAmount==null?"—":money.format(goal.remainingAmount)}</strong></div><div><span>Al mes</span><strong>{goal.monthlyRequired==null?"—":money.format(goal.monthlyRequired)}</strong></div></div>
          ...
          {goal.progressMode==="account"&&goal.currentAmount!=null&&goal.currentAmount<0&&<p className="goal-source-note">El saldo real es {money.format(goal.currentAmount)}. Para el porcentaje de progreso se usa 0 €, sin ocultar el saldo negativo.</p>}
        </article>})}</div>}
    </section>
