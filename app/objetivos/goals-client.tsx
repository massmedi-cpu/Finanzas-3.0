...
const statusTone:Record<GoalStatus,"info"|"warning"|"error"|"ok"|"muted">={on_track:"info",attention:"warning",overdue:"error",achieved:"ok",flexible:"muted",source_missing:"error"};
...
export function GoalsClient({initialData}:{initialData:GoalsOverview}){
  const [data,setData]=useState(initialData);const [editor,setEditor]=useState<Editor|null>(null);const [loading,setLoading]=useState(false);const [feedback,setFeedback]=useState<Feedback|null>(null);
  ...
  async function saveGoal(){
    ...
    if(!editor.name.trim()||!Number.isFinite(targetAmount)||targetAmount<=0){setFeedback(failure("Indica un nombre y un importe objetivo válido."));return;}
    if(editor.progressMode==="manual"&&(!Number.isFinite(manualAmount)||manualAmount<0)){setFeedback(failure("El progreso manual debe ser un importe válido."));return;}
    if(editor.progressMode==="account"&&!editor.accountId){setFeedback(failure("Selecciona la cuenta que medirá el progreso."));return;}
    ...
    try{
      ...
      const j=await r.json();if(!r.ok)throw new Error(j.error||"No se ha podido guardar el objetivo");setEditor(null);await reload();setFeedback(success("Objetivo guardado."));
    }catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido guardar el objetivo"));}finally{setLoading(false);}
  }
  async function removeGoal(id:string){
    if(!window.confirm("¿Archivar este objetivo? No se modifica ningún movimiento ni saldo bancario."))return;
    setLoading(true);setFeedback(null);try{const r=await fetch(`/api/goals?id=${encodeURIComponent(id)}`,{method:"DELETE"});const j=await r.json();if(!r.ok)throw new Error(j.error||"No se ha podido archivar");setEditor(null);await reload();setFeedback(success("Objetivo archivado."));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido archivar"));}finally{setLoading(false);}
  }

  return <div className={`goals-module ${loading?"is-loading":""}`} aria-busy={loading?"true":undefined}>
    ...
    {editor&&<div className="goals-modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!loading)setEditor(null)}}><section className="goals-modal" role="dialog" aria-modal="true" aria-labelledby="goal-editor-title" aria-busy={loading?"true":undefined}>
      <div className="goals-modal-head"><div><p className="eyebrow">{editor.id?"EDITAR META":"NUEVA META"}</p><h2 id="goal-editor-title">Objetivo financiero</h2></div><button className="icon-button" type="button" onClick={()=>setEditor(null)} aria-label="Cerrar" disabled={loading}>×</button></div>
