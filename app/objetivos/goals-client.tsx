...
export function GoalsClient({initialData}:{initialData:GoalsOverview}){
  ...
  return <div className={`goals-module ${loading?"is-loading":""}`}>
    ...
    <section className="goals-summary" aria-label="Resumen de objetivos">
      <article><span>Objetivo total</span><strong>{data.summary.activeCount?formatEuro(data.summary.targetTotal):"—"}</strong><small>{data.summary.activeCount?`${knownProgress.toLocaleString("es-ES",{maximumFractionDigits:1})}% registrado`:"Crea tu primera meta"}</small></article>
