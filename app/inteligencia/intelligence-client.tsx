...
export function IntelligenceClient({initialData}:{initialData:ActionableIntelligence}){
  ...
  async function act(key:string,action:Action){
    ...
    try{
      ...
      setFeedback({tone:"success",message:action==="snoozed"?"Señal pospuesta 7 días.":action==="resolved"?"Señal marcada como revisada.":"Señal ocultada."});
      router.refresh();
    }catch(error){setFeedback({tone:"error",message:error instanceof Error?error.message:"No se pudo actualizar la señal"});}
    finally{setBusy(null);}
  }

  const Actions=({keyValue,state,resolve=false,dismiss=false}:{keyValue:string;state:ActionableState;resolve?:boolean;dismiss?:boolean})=><div className="intelligence-actions" aria-busy={busy===keyValue?"true":"false"}>
    <button className="text-button" type="button" disabled={busy===keyValue||state==="snoozed"} onClick={()=>act(keyValue,"snoozed")}>Posponer 7 días</button>
    {resolve&&<button className="secondary-action" type="button" disabled={busy===keyValue} onClick={()=>act(keyValue,"resolved")}>Marcar revisado</button>}
    {dismiss&&<button className="text-button muted" type="button" disabled={busy===keyValue} onClick={()=>act(keyValue,"dismissed")}>Ocultar</button>}
  </div>;

  const Empty=({children}:{children:string})=><div className="empty-state"><strong>Sin señales.</strong><span>{children}</span></div>;

  return <div className="intelligence-sections">
    {feedback&&<p className={`inline-alert ${feedback.tone}`} role={feedback.tone==="error"?"alert":"status"} aria-live="polite">{feedback.message}</p>}
    ...
    <section className="panel intelligence-section" aria-labelledby="intelligence-rising"><div className="panel-head"><div><p className="eyebrow">TENDENCIA</p><h2 id="intelligence-rising">Categorías que aceleran</h2></div><span className="pill">{formatInteger(initialData.rising.length)}</span></div>
      {initialData.rising.length?<div className="intelligence-list">{initialData.rising.map((item:RisingSignal)=><article key={item.key} className={`intelligence-item severity-${item.severity}`}><div className="intelligence-item-main"><div><span>2 meses completos vs. 2 anteriores</span><strong>{item.category}</strong><small>{formatInteger(item.recentTransactions)} movimientos recientes · {formatInteger(item.previousTransactions)} anteriores</small></div><div className="intelligence-value"><strong>{formatSignedPercent(item.changeRatio*100,1)}</strong><small>+{formatEuro(item.difference)}</small></div></div><p>El bloque reciente suma {formatEuro(item.recentSpend)} frente a {formatEuro(item.previousSpend)} en el bloque comparable anterior.</p><div className="intelligence-item-footer"><Link className="text-button button-link" href={item.href}>Ver categoría</Link><Actions keyValue={item.key} state={item.state}/></div></article>)}</div>:<Empty>No hay categorías con una subida material y suficientemente respaldada por dos periodos completos comparables.</Empty>}
    </section>

    <section className="panel intelligence-section" aria-labelledby="intelligence-opportunities"><div className="panel-head"><div><p className="eyebrow">ESCENARIOS DE AHORRO</p><h2 id="intelligence-opportunities">Qué supondría reducir un 10%</h2><p className="decision-note">Es una simulación sobre gasto observado, no un objetivo ni una recomendación automática.</p></div><span className="pill">{formatInteger(initialData.opportunities.length)}</span></div>
      {initialData.opportunities.length?<div className="intelligence-list">{initialData.opportunities.map((item:OpportunitySignal)=><article key={item.key} className="intelligence-item severity-low"><div className="intelligence-item-main"><div><span>{item.category}</span><strong>{formatEuro(item.monthlyAverage)}<small>/mes de media</small></strong></div><div className="intelligence-value"><strong>{formatEuro(item.monthlyScenarioSavings)}<small>/mes</small></strong><small>{formatEuro(item.annualScenarioSavings)} al año</small></div></div><div className="intelligence-item-footer"><Link className="text-button button-link" href={item.href}>Revisar gastos</Link><Actions keyValue={item.key} state={item.state} dismiss/></div></article>)}</div>:<Empty>No hay categorías discrecionales con muestra suficiente para construir un escenario útil.</Empty>}
    </section>
  </div>;
}
