...
const sections: Record<string,{title:string;description:string;items:string[]}> = {
  ...
  configuracion:{title:"Configuración",description:"Aplicación, datos, cuenta, apariencia, preferencias y sistema.",items:["Versión 0.1.0","Fuente Google Sheets solo lectura","Entorno y estado de sincronización"]},
};
...
export default async function SectionPage({params}:{params:Promise<{section:string}>}){
  ...
  if(!data) notFound();
  return <main className="section-page"><Link href="/">← Volver a Inicio</Link><h1>{data.title}</h1><p>{data.description}</p><article className="section-card"><h2>Base funcional definida</h2><ul>{data.items.map(item=><li key={item}>{item}</li>)}</ul><p>Esta sección se conectará progresivamente al núcleo de datos sin modificar las reglas ya validadas.</p></article></main>;
}
