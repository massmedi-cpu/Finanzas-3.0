...
export default async function PrevisionPage() {
  ...
  const activeScenarios = baseDate ? scenarios.filter((scenario) => scenario.active).map((scenario) => simulateScenario(forecast, knownBalance, baseDate as string, scenario)) : [];
  ...
  return (
    <main className="page">
      ...
      {dataError ? <div className="status-panel status-danger"><div><div className="status-title">No se puede calcular una previsión segura</div><div className="status-copy">La previsión queda bloqueada si falta la fuente, tus ajustes privados o las preferencias recurrentes.</div></div></div> : !baseDate ? <section className="card"><div className="empty">La previsión se activará cuando exista histórico bancario sincronizado.</div></section> : <>
        ...
        {activeScenarios.length > 0 && <section className="card section-gap scenario-comparison"><div className="card-heading-row"><div><div className="eyebrow">Simulador what-if</div><h2 className="section-title">Comparativa de escenarios activos</h2></div><span className="badge">{activeScenarios.length} escenarios</span></div><div className="scenario-grid">{activeScenarios.map((scenario) => <article className="scenario-card" key={scenario.scenarioId}><div className="row-title">{scenario.name}</div><div className="scenario-balance">{euro.format(scenario.projectedBalance)}</div><div className={`scenario-delta ${scenario.differenceVsBaseline < 0 ? 'amount-negative' : 'amount-positive'}`}>{scenario.differenceVsBaseline >= 0 ? '+' : ''}{euro.format(scenario.differenceVsBaseline)} frente al escenario base</div><div className="scenario-meta"><span>Horizonte</span><strong>{scenario.horizonDate}</strong></div><div className="scenario-meta"><span>Ahorro reservado</span><strong>{euro.format(scenario.savingsAllocated)}</strong></div><div className="scenario-meta"><span>Liquidez libre tras ahorro</span><strong>{euro.format(scenario.freeAfterSavings)}</strong></div></article>)}</div></section>}
