...
import { buildForecastCalendar, buildForecastYearlyOutlook } from '../../src/domain/forecast-calendar-engine';
...
export default async function PrevisionPage() {
  ...
  const yearlyOutlook = baseDate && forecastHorizonDate && forecastHorizonMonths > 12
    ? buildForecastYearlyOutlook(forecast, knownBalance, baseDate, forecastHorizonDate)
    : [];
  ...
  return (
    <main className="page">
      ...
      {dataError ? <div className="status-panel status-danger"><div><div className="status-title">No se puede calcular una previsión segura</div><div className="status-copy">La previsión queda bloqueada si falta la fuente, tus ajustes privados o las preferencias recurrentes.</div></div></div> : !baseDate ? <section className="card"><div className="empty">La previsión se activará cuando exista histórico bancario sincronizado.</div></section> : <>
        ...
        {yearlyOutlook.length > 0 && <section className="section-gap"><div className="card-heading-row"><div><div className="eyebrow">Horizonte largo</div><h2 className="section-title">Evolución anual prevista</h2></div><span className="badge">Hasta {forecastHorizonDate}</span></div><div className="grid grid-3">{yearlyOutlook.map((year) => <article className={`card${year.firstNegativeDate ? ' risk-card' : ''}`} key={year.year}><div className="metric-label">{year.year}</div><div className={`metric-value metric-value-small ${year.netCashFlow < 0 ? 'amount-negative' : 'amount-positive'}`}>{year.netCashFlow >= 0 ? '+' : ''}{euro.format(year.netCashFlow)}</div><p className="metric-note">Ingresos {euro.format(year.income)} · gastos {euro.format(year.expenses)}</p><div className="scenario-meta"><span>Saldo final</span><strong className={year.endingBalance < 0 ? 'amount-negative' : ''}>{euro.format(year.endingBalance)}</strong></div><div className="scenario-meta"><span>Movimientos previstos</span><strong>{year.movementCount}</strong></div>{year.firstNegativeDate && <div className="forecast-calendar-risk-note">Saldo negativo desde {year.firstNegativeDate}</div>}</article>)}</div><p className="metric-note">El primer y último año pueden ser periodos parciales. No se añaden movimientos fuera del horizonte configurado.</p></section>}
