    ...
    <main className="page">
      <section className="page-header">
        <div>
          ...
          <p className="subtitle">La previsión se construye con recurrencias observadas en movimientos reales y respeta tus exclusiones internas; no se inventan cargos ni ingresos futuros.</p>
        </div>
        ...
        <>
          <section className="grid grid-3">
            {horizons.map(([label, date]) => {
              ...
              return <article className="card" key={label}><div className="metric-label">Saldo conocido proyectado · {label}</div><div className="metric-value">{euro.format(projected)}</div><p className="metric-note">Hasta {date} · basado en recurrencias detectadas</p></article>;
            })}
          </section>

          <section className="grid grid-2 section-gap">
            <article className="card">
              <div className="card-heading-row"><div><div className="eyebrow">Calendario financiero</div><h2 className="section-title">Próximos movimientos esperados</h2></div></div>
              {upcoming.length === 0 ? <div className="empty compact-empty">No hay recurrencias suficientemente fiables todavía.</div> : (
                <div className="stack">{upcoming.map((movement) => <div className="row" key={movement.id}><div><div className="row-title">{movement.description}</div><div className="row-meta">{movement.expectedDate} · {movement.category || 'Sin categoría'} · confianza {Math.round(movement.confidence * 100)}%</div></div><div className={`amount ${movement.amount < 0 ? 'amount-negative' : 'amount-positive'}`}>{euro.format(movement.amount)}</div></div>)}</div>
              )}
            </article>
