...
export default async function PlanPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <main className="app-shell"><section id="main-content" tabIndex={-1} className="workspace plan-workspace">
    ...
    <section className="plan-domains" aria-labelledby="domains-title"><div className="plan-section-head"><div><p className="eyebrow">LAS CINCO FUENTES DEL PLAN</p><h2 id="domains-title">Detalle por dominio</h2></div><p>El Plan resume; cada módulo sigue siendo la fuente operativa de su área.</p></div><div className="plan-domain-grid">
      <Link href={d.budget.href}><span>Presupuesto</span><strong>{formatEuro(d.budget.available)}</strong><small>disponible · {formatInteger(d.budget.overBudgetCount)} excedidos</small><progress max={100} value={budgetPercent}/><em>{Math.round(budgetPercent)}% consumido</em></Link>
      <Link href={d.forecast.href}><span>Previsión</span><strong>{formatEuro(d.forecast.lowestBalance)}</strong><small>saldo mínimo en 90 días</small><em>{formatInteger(d.forecast.eventCount)} confirmados · {formatInteger(d.forecast.suggestionCount)} sugerencias</em></Link>
      <Link href={d.goals.href}><span>Objetivos</span><strong>{formatEuro(goals.remainingTotal)}</strong><small>pendiente total · {formatInteger(goals.activeCount)} activos</small><em>{formatInteger(goals.achievedCount)} alcanzados · {formatInteger(goals.overdueCount)} vencidos</em></Link>
      <Link href={d.netWorth.href}><span>Patrimonio</span><strong>{formatEuro(d.netWorth.netWorth)}</strong><small>{formatEuro(d.netWorth.assets)} activos · {formatEuro(d.netWorth.liabilities)} pasivos</small><em>{d.netWorth.coverage.currentComplete?"Cobertura completa":"Cobertura incompleta"}</em></Link>
      <Link href={d.control.href}><span>Control</span><strong>{formatInteger(d.control.visibleAlertCount)}</strong><small>alertas visibles · {formatInteger(d.control.snapshot.closeBlockers)} bloqueos</small><em>{d.control.snapshot.closeReady?"Mes listo para cerrar":"Revisión pendiente"}</em></Link>
    </div></section>
