...
const emptyPrivateState: Awaited<ReturnType<typeof getPrivateState>> = { overrides: [], budgets: [], goals: [], futureEvents: [], scenarios: [] };
...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  if (!isGoogleSheetsConfigured()) {
    return (
      <main className="page">
        <section className="page-header"><div><div className="eyebrow">Plan financiero</div><h1>Control mensual 360º</h1><p className="subtitle">El plan se activará cuando exista una fuente bancaria válida.</p></div><span className="badge">V2.0.1</span></section>
  ...
  try {
    const [sourceResult, privateResult, splitsResult, recurringResult] = await Promise.allSettled([
      loadValidatedSource(),
      getPrivateState(),
      getMovementSplits(),
    ...
    const privateState = privateResult.status === 'fulfilled' ? privateResult.value : emptyPrivateState;
    const splits = splitsResult.status === 'fulfilled' ? splitsResult.value : [];
    const recurringPreferences = recurringResult.status === 'fulfilled' ? recurringResult.value : [];
    ...
    const duplicateGroups = findDuplicateCandidates(monthSourceRows).length;
    ...
    const forecast = latestDate
      ? combineForecasts(buildForecast(patterns, latestDate, 120), expandPlannedEvents(privateState.futureEvents, latestDate, 120))
      : [];
    ...
    const insights = buildFinancialInsights({
      ...
      previous: previousSummary.transactionCount > 0 ? previousSummary : null,
    ...
    return (
      <main className="page">
        <section className="page-header plan-header">
          <div>
            <div className="eyebrow">Plan financiero</div>
          ...
          <span className="badge">V2.0.1 · {selectedMonth}</span>
        </section>
        ...
        <section className="grid grid-2 section-gap">
          <article className="card">
            ...
            <div className="plan-budget-grid">
              ...
              <div><span>Remanente recibido</span><strong>{euro.format(budgetSummary.carryIn)}</strong></div>
              <div className={budgetSummary.overspent > 0 ? 'plan-risk' : ''}><span>Sobregasto</span><strong>{euro.format(budgetSummary.overspent)}</strong></div>
            </div>
            {envelopes.length > 0 && <div className="stack plan-envelope-list">{envelopes.slice(0, 5).map((envelope) => <div className="row" key={envelope.category}><div><div className="row-title">{envelope.category}</div><div className="row-meta">Asignado {euro.format(envelope.assigned + envelope.carryIn)} · gastado {euro.format(envelope.spent)}</div></div><strong className={envelope.available < 0 ? 'amount-negative' : 'amount-positive'}>{euro.format(envelope.available)}</strong></div>)}</div>}
          </article>
        ...
        <section className="grid grid-2 section-gap">
          <article className="card">
            <div className="card-heading-row"><div><div className="eyebrow">Análisis explicable</div><h2 className="section-title">Qué merece tu atención</h2></div><span className="badge">Sin inventar datos</span></div>
            <div className="insight-list">
              {insights.length === 0 ? <div className="empty compact-empty">No hay suficientes datos para generar conclusiones fiables.</div> : insights.map((insight) => (
                <div className={`insight insight-${insight.severity}`} key={insight.id}>
                  <div className="insight-top"><span>{severityLabel(insight.severity)}</span><strong>{insight.title}</strong></div>
                  <p>{insight.message}</p>
                  <small>Base: {insight.evidence}</small>
                </div>
              ))}
            </div>
          </article>
          ...
          <article className="card">
            ...
            <div className="alert-summary alert-summary-3">
              ...
              <div className={`alert-stat${budgetSummary.overspent > 0 ? ' alert-stat-risk' : ''}`}><strong>{budgetSummary.overspent > 0 ? '!' : '✓'}</strong><span>{budgetSummary.overspent > 0 ? 'sobregasto en sobres' : 'presupuesto sin sobregasto'}</span></div>
            </div>
            <div className="stack plan-goal-list">
              {activeGoals.length === 0 ? <div className="empty compact-empty">No hay objetivos activos.</div> : activeGoals.map((goal) => {
  ...
  return (
    <main className="page">
      <section className="page-header"><div><div className="eyebrow">Plan financiero</div><h1>Control mensual 360º</h1></div><span className="badge">V2.0.1</span></section>
