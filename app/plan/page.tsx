...
export default async function PlanPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <main className="app-shell"><section id="main-content" tabIndex={-1} className="workspace plan-workspace">
    ...
    <section className="plan-domains" aria-labelledby="domains-title"><div className="plan-section-head"><div><p className="eyebrow">LAS CINCO FUENTES DEL PLAN</p><h2 id="domains-title">Detalle por dominio</h2></div><p>El Plan resume; cada módulo sigue siendo la fuente operativa de su área.</p></div><div className="plan-domain-grid">
      <Link href={d.budget.href}><span>Presupuesto</span><strong>{formatEuro(d.budget.available)}</strong><small>disponible · {d.budget.overBudgetCount} excedidos</small><progress max={100} value={budgetPercent}/><em>{Math.round(budgetPercent)}% consumido</em></Link>
      <Link href={d.forecast.href}><span>Previsión</span><strong>{formatEuro(d.forecast.lowestBalance)}</strong><small>saldo mínimo en 90 días</small><em>{d.forecast.eventCount} confirmados · {d.forecast.suggestionCount} sugerencias</em></Link>
      <Link href={d.goals.href}><span>Objetivos</span><strong>{formatEuro(goals.remainingTotal)}</strong><small>pendiente total · {goals.activeCount} activos</small><em>{goals.achievedCount} alcanzados · {goals.overdueCount} vencidos</em></Link>
