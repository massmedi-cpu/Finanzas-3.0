...
export default async function LongHorizonPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <main className="app-shell"><section id="main-content" tabIndex={-1} className="workspace plan-workspace horizon-workspace">
    ...
    <section className={`horizon-status horizon-status-${horizon.sustainability}`}><div><p className="eyebrow">SOSTENIBILIDAD</p><h2>{sustainabilityCopy[horizon.sustainability]}</h2><p>Capacidad de referencia {money.format(horizon.monthlyCapacityReference)}/mes · objetivos {money.format(horizon.monthlyGoalCommitment)}/mes · margen {money.format(horizon.monthlyResidualCapacity)}/mes.</p></div><Link className="ghost button-link" href={`/plan?month=${plan.month}`}>Volver al Plan</Link></section>

    <section className="horizon-points" aria-label="Horizonte lineal de capacidad">{horizon.points.map(point=><article key={point.months}><span>{monthLabel(point.months)}</span><strong className={point.residualCapacity<0?"negative":"positive"}>{money.format(point.residualCapacity)}</strong><small>margen lineal tras objetivos</small><dl><div><dt>Capacidad de referencia</dt><dd>{money.format(point.capacityReference)}</dd></div><div><dt>Esfuerzo de objetivos</dt><dd>{money.format(point.goalCommitment)}</dd></div></dl><p>No es saldo bancario: es capacidad acumulada si la referencia mensual se mantuviera.</p></article>)}</section>

    <div className="horizon-grid">
      <article className="panel horizon-liquidity"><div className="panel-head"><div><p className="eyebrow">LÍMITE DE PREVISIÓN</p><h2>Liquidez · 90 días</h2></div><Link className="pill pill-link" href="/prevision">Abrir Previsión</Link></div><dl><div><dt>Saldo actual</dt><dd>{money.format(horizon.liquidityBoundary.currentBalance)}</dd></div><div><dt>Saldo previsto a 90 días</dt><dd className={horizon.liquidityBoundary.projectedBalance90<0?"negative":""}>{money.format(horizon.liquidityBoundary.projectedBalance90)}</dd></div><div><dt>Mínimo previsto</dt><dd className={horizon.liquidityBoundary.lowestBalance90<0?"negative":""}>{money.format(horizon.liquidityBoundary.lowestBalance90)}</dd></div><div><dt>Primer negativo</dt><dd>{horizon.liquidityBoundary.firstNegativeDate?date.format(new Date(`${horizon.liquidityBoundary.firstNegativeDate}T12:00:00`)):"No detectado"}</dd></div></dl><p className="horizon-note">Financial App no extrapola este saldo a 6 o 12 meses porque los eventos confirmados de Previsión solo tienen cobertura canónica de 90 días.</p></article>

      <article className="panel horizon-goals"><div className="panel-head"><div><p className="eyebrow">OBJETIVOS</p><h2>Esfuerzo pendiente</h2></div><Link className="pill pill-link" href="/objetivos">Gestionar objetivos</Link></div><dl><div><dt>Pendiente total</dt><dd>{money.format(horizon.goalRemaining)}</dd></div><div><dt>Ritmo mensual requerido</dt><dd>{money.format(horizon.monthlyGoalCommitment)}</dd></div><div><dt>Meses matemáticos al ritmo actual</dt><dd>{horizon.goalFundingMonths==null?"—":number.format(horizon.goalFundingMonths)}</dd></div><div><dt>Margen mensual tras objetivos</dt><dd className={horizon.monthlyResidualCapacity<0?"negative":"positive"}>{money.format(horizon.monthlyResidualCapacity)}</dd></div></dl><p className="horizon-note">Los meses matemáticos no sustituyen las fechas objetivo individuales: solo relacionan pendiente total y esfuerzo mensual requerido.</p></article>
    </div>
