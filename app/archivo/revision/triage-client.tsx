  ...
  return <div className="triage-queue">
    ...
    <div className="triage-list">
      {visibleDocuments.map(item=>{
        const suggestions=[...item.suggestions].sort((a,b)=>Number(b.score||0)-Number(a.score||0));
        const archiveQuery=`/archivo?view=new&q=${encodeURIComponent(item.fileName)}`;
        const canMatch=item.action==="ready_to_link"||item.action==="review_match";
        const selectedSafe=selected.includes(item.id);
        const isActive=item.id===activeId;
        return <section id={`triage-${item.id}`} className={`triage-item triage-${item.action}${item.safeOperation?" triage-safe":""}${isActive?" triage-active":""}`} key={item.id} aria-current={isActive?"step":undefined}>
          <header><div className="triage-document-heading">{item.safeOperation&&<label className="operation-check"><input type="checkbox" checked={selectedSafe} onChange={()=>toggle(item.id)} disabled={busy!==null}/><span className="sr-only">Seleccionar operación segura para {item.fileName}</span></label>}<div><div className="triage-item-title"><span className="triage-priority">Prioridad {item.priorityScore}</span><span className="triage-action">{actionLabel(item.action)}</span>{item.safeOperation&&<span className="operation-safe-badge">Seguro y reversible</span>}</div><h2>{item.fileName}</h2><p>{item.merchant||"Emisor sin identificar"} · {date(item.documentDate)} · {money(item.amount)}</p></div></div><div className="triage-header-actions">{!isActive&&<button className="primary-action" type="button" onClick={()=>focusDocument(item.id)}>Revisar</button>}{item.storageUrl&&<a className="ghost button-link" href={item.storageUrl} target="_blank" rel="noreferrer">Original</a>}<Link className="ghost button-link" href={archiveQuery}>Abrir en Archivo</Link></div></header>
          {isActive&&<div className="triage-detail">
            <p className="triage-hint">{actionHint(item.action)}</p>
            {item.safeOperation&&<div className="operation-safe-note"><strong>{item.safeOperation.label}</strong><span>Solo se ejecutará si al confirmar sigue cumpliendo exactamente las condiciones de seguridad.</span></div>}
            {!!item.reasons.length&&<ul className="triage-reasons">{item.reasons.map(reason=><li key={reason}>{reason}</li>)}</ul>}
            {canMatch&&suggestions.length>0&&<div className="review-candidates">{suggestions.map((candidate,index)=><article key={`${item.id}-${candidate.sourceId}`} className={`review-candidate${index===0?" best":""}`}><div><div className="candidate-heading"><strong>{index===0?"Mejor coincidencia":"Alternativa"}</strong><span className={`confidence confidence-${candidate.confidenceTier||"unknown"}`}>Confianza {confidenceLabel(candidate.confidenceTier).toLowerCase()}</span></div><span>{candidate.counterparty||candidate.concept||candidate.sourceId}</span><small>{date(candidate.date)} · {money(candidate.amount)}{candidate.daysDiff!=null?` · ${Math.abs(candidate.daysDiff)} día${Math.abs(candidate.daysDiff)===1?"":"s"} de diferencia`:""}{candidate.amountDiff!=null?` · diferencia ${money(Math.abs(candidate.amountDiff))}`:""}</small>{candidate.reasons?.length?<small>{candidate.reasons.join(" · ")}</small>:null}</div><button className={index===0?"primary-action":"ghost"} type="button" disabled={busy!==null} onClick={()=>link(item.id,candidate)}>{busy===item.id?"Asociando…":"Asociar manualmente"}</button></article>)}</div>}
