...
const warning=(message:string):Feedback=>({tone:"warning",message});
...
export function ControlClient({initialData}:{initialData:ControlOverview}){
  const [data,setData]=useState(initialData);const [loading,setLoading]=useState(false);const [feedback,setFeedback]=useState<Feedback|null>(null);const [notes,setNotes]=useState("");
  ...
  async function alertAction(alert:ControlAlert,action:ControlAlertState){setLoading(true);setFeedback(null);try{const r=await fetch("/api/control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({kind:"alert",key:alert.key,action,days:7})});const j=await r.json();if(!r.ok)throw new Error(j.error||"No se ha podido actualizar el aviso");await reload();setFeedback(success(action==="resolved"?"Aviso marcado como resuelto.":action==="snoozed"?"Aviso pospuesto 7 días.":action==="dismissed"?"Aviso ignorado.":"Aviso reabierto."));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido actualizar el aviso"));}finally{setLoading(false);}}
  async function closeMonth(){if(!isCompletedMonth)return;if(!snapshot.closeReady){setFeedback(warning("Este mes todavía tiene bloqueos que deben resolverse antes del cierre."));return;}if(!window.confirm(`¿Cerrar ${monthLabel(data.month)}? Se guardará una fotografía verificable de sus cifras y avisos.`))return;setLoading(true);setFeedback(null);try{const r=await fetch("/api/control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({kind:"close",month:data.month,notes})});const j=await r.json();if(!r.ok)throw new Error(j.error==="month_has_blockers"?"El mes tiene bloqueos pendientes.":j.error||"No se ha podido cerrar el mes");setNotes("");await reload();setFeedback(success(`Cierre de ${monthLabel(data.month)} guardado.`));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido cerrar el mes"));}finally{setLoading(false);}}
  async function reopenMonth(){if(!selectedClose||selectedClose.status!=="closed")return;if(!window.confirm(`¿Reabrir ${monthLabel(data.month)}? El snapshot histórico se conserva, pero el mes volverá a quedar editable para un nuevo cierre.`))return;setLoading(true);setFeedback(null);try{const r=await fetch("/api/control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({kind:"reopen",month:data.month})});const j=await r.json();if(!r.ok)throw new Error(j.error||"No se ha podido reabrir el mes");await reload();setFeedback(success(`Mes ${monthLabel(data.month)} reabierto.`));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido reabrir el mes"));}finally{setLoading(false);}}

  return <div className={`control-module ${loading?"is-loading":""}`} aria-busy={loading?"true":undefined}>
