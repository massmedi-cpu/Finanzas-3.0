...
import { getSystemHealthSnapshot } from "@/lib/financial/system-health";
...
type HealthData=Awaited<ReturnType<typeof getSystemHealthSnapshot>>;
...
function ControlStreamFallback({title}:{title:string}){
  return <section className="control-panel" aria-busy="true" aria-label={`Cargando ${title}`}><div className="control-panel-head"><div><p className="eyebrow">ACTUALIZANDO</p><h2>{title}</h2></div></div><p className="muted-copy">Cargando este diagnóstico sin bloquear los avisos ni el cierre mensual.</p></section>;
}
function DiagnosticUnavailable({title,error}:{title:string;error:string}){console.error(`[control] ${title} unavailable`,error);return <section className="control-panel" role="status" aria-label={`${title} no disponible`}><div className="control-panel-head"><div><p className="eyebrow">DIAGNÓSTICO AISLADO</p><h2>{title}</h2></div></div><p className="muted-copy">Este diagnóstico no está disponible temporalmente. El resto del Centro de control continúa operativo.</p></section>;}
async function HealthStream({data}:{data:Promise<DiagnosticResult<HealthData>>}){const result=await data;return result.ok?<SystemHealthPanel data={result.data}/>:<DiagnosticUnavailable title="Estado operativo" error={result.error}/>;}
...
export default async function ControlPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const healthPromise=settleDiagnostic(getSystemHealthSnapshot());
  ...
  return <main className="app-shell"><section id="main-content" tabIndex={-1} className="workspace control-workspace">
    <header className="topbar"><div><p className="eyebrow">CENTRO DE CONTROL · {data.version}</p><h1>Control financiero</h1><p>Problemas reales, prioridades, cierre mensual, frescura de datos e integridad técnica sobre una única base financiera.</p></div></header>
    <ControlClient initialData={data}/>
    <Suspense fallback={<ControlStreamFallback title="Estado operativo"/>}><HealthStream data={healthPromise}/></Suspense>
