        ...
        <>
          ...
          <section className="grid grid-4">
            <article className="card">
              <div className="metric-label">Ingresos {selectedYear}</div>
              <div className="metric-value amount-positive">{euro.format(yearly.income)}</div>
              <p className="metric-note">{incomeDelta && previousYear ? <><span className={incomeDelta.className}>{incomeDelta.text}</span> frente a {previousYear} hasta el mismo mes</> : 'Ingresos netos de traspasos'}</p>
            </article>
            <article className="card">
              <div className="metric-label">Gastos {selectedYear}</div>
              <div className="metric-value amount-negative">{euro.format(yearly.expenses)}</div>
              <p className="metric-note">{expenseDelta && previousYear ? <><span className={expenseDelta.className}>{expenseDelta.text}</span> frente a {previousYear} hasta el mismo mes</> : 'Gasto real acumulado'}</p>
            </article>
            <article className="card">
              <div className="metric-label">Cash flow {selectedYear}</div>
              <div className={`metric-value ${yearly.net < 0 ? 'amount-negative' : 'amount-positive'}`}>{euro.format(yearly.net)}</div>
              <p className="metric-note">{comparison ? `${comparison.netDelta >= 0 ? '+' : ''}${euro.format(comparison.netDelta)} frente a ${previousYear}` : 'Ingresos menos gastos'}</p>
            </article>
            <article className="card">
              <div className="metric-label">Tasa de ahorro</div>
              <div className={`metric-value ${yearly.savingsRate < 0 ? 'amount-negative' : 'amount-positive'}`}>{percent.format(yearly.savingsRate)}%</div>
              <p className="metric-note">{yearly.transactions} movimientos · {yearly.transfersExcluded} traspasos excluidos</p>
            </article>
          </section>

          <section className="card section-gap">
            <div className="card-heading-row">
              <div><div className="eyebrow">Cash flow anual conciliado</div><h2 className="section-title">Ingresos, gastos y acumulado por mes</h2></div>
              <span className="badge">{monthly.length} meses con datos</span>
            </div>
            <CashFlowChart rows={monthly} />
          </section>

          <section className="grid grid-4 section-gap">
            {quarterly.map((item) => (
              <article className="card quarter-card" key={item.quarter}>
                <div className="card-heading-row"><div className="metric-label">{item.quarter}</div><span className="row-meta">{item.transactions ? `${item.transactions} mov.` : 'Sin datos'}</span></div>
                {item.transactions ? (
                  <>
                    <div className={`quarter-net ${item.net < 0 ? 'amount-negative' : 'amount-positive'}`}>{euro.format(item.net)}</div>
                    <div className="quarter-meta"><span>Ingresos <strong>{euro.format(item.income)}</strong></span><span>Gastos <strong>{euro.format(item.expenses)}</strong></span></div>
                  </>
          ...
          <section className="grid grid-2 section-gap">
            <article className="card">
              <div className="card-heading-row"><div><div className="eyebrow">Gasto por categoría</div><h2 className="section-title">Dónde se va el dinero</h2></div><span className="badge">Top {categories.length}</span></div>
              {categories.length === 0 ? <div className="empty compact-empty">No hay gastos categorizados en este periodo.</div> : (
            ...
            <article className="card table-card">
              <div className="card-heading-row"><div><div className="eyebrow">Detalle mensual</div><h2 className="section-title">Mes a mes</h2></div><span className="badge">{selectedYear}</span></div>
              <div className="table-scroll">
                <table className="data-table report-table">
                  <thead><tr><th>Mes</th><th className="numeric">Ingresos</th><th className="numeric">Gastos</th><th className="numeric">Cash flow</th><th className="numeric">Acumulado</th></tr></thead>
                  <tbody>
                    {monthly.map((item) => <tr key={item.month}><td className="table-primary">{item.month}</td><td className="numeric amount-positive">{euro.format(item.income)}</td><td className="numeric amount-negative">{euro.format(item.expenses)}</td><td className={`numeric amount ${item.net < 0 ? 'amount-negative' : 'amount-positive'}`}>{euro.format(item.net)}</td><td className={`numeric amount ${item.cumulativeNet < 0 ? 'amount-negative' : 'amount-positive'}`}>{euro.format(item.cumulativeNet)}</td></tr>)}
                  </tbody>
                </table>
              </div>
            </article>
