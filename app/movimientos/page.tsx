...
import { BulkUndoButton } from "./bulk-undo-button";
export const dynamic="force-dynamic";
export default async function MovementsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){await requireAuthorizedUser();const params=await searchParams;const initialFilters=parseMovementSearchParams(params);const initialData=await getMovements(movementFiltersForData(initialFilters));const newIds=initialData.items.filter(item=>item.status==="new").map(item=>item.id);return <main className="app-shell"><section id="main-content" tabIndex={-1} className="workspace movements-workspace"><header className="topbar movements-heading"><div><p className="eyebrow">MOVIMIENTOS · {initialData.version||APP_VERSION}</p><h1>Movimientos</h1><p>Busca, filtra, revisa, divide y concilia sin modificar nunca el dato bancario original.</p></div><div className="topbar-actions"><BulkUndoButton/><Link className="ghost button-link" href="/movimientos/conciliacion">Conciliación</Link><Link className="ghost button-link" href="/">Volver al inicio</Link></div></header><AcknowledgeNewMovements ids={newIds}/><MovementsClient initialData={initialData} initialFilters={initialFilters}/></section></main>}
