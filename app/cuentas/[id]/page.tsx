  ...
  return <main className="app-shell">
    ...
    <section className="workspace account-detail-workspace">
      ...
      <div className="account-detail-hero">
        <div><span>Saldo actual</span><strong>{account.balance == null ? "—" : money.format(account.balance)}</strong><small>{account.balanceDate ? `Saldo a ${date.format(new Date(account.balanceDate + "T12:00:00"))}` : "Sin fecha de saldo"}</small></div>
      ...
      <article className="panel recent-account-movements">
        ...
        <div className="recent-movement-list">
          {data.recentMovements.map(movement => <div className="recent-movement" key={movement.id}>
            ...
            <strong className={`recent-amount ${movement.amount < 0 ? "negative" : "positive"}`}>{money.format(movement.amount)}</strong>
          </div>)}
        </div>
      </article>
    </section>
