...
const info=(message:string):Feedback=>({tone:"info",message});
type Editor={
...
export function RulesClient({initialData}:{initialData:RulesOverview}){
  const [data,setData]=useState(initialData);const [editor,setEditor]=useState<Editor|null>(null);const [preview,setPreview]=useState<RulePreview|null>(null);const [loading,setLoading]=useState(false);const [feedback,setFeedback]=useState<Feedback|null>(null);
  ...
  async function previewEditor(next=editor){if(!next)return;const error=validate(next);if(error){setFeedback(failure(error));return;}setLoading(true);setFeedback(null);try{const j=await post({kind:"preview",rule:payload(next)});setPreview(j);if(!j.changeable)setFeedback(info(j.matched?"La regla coincide, pero no sobrescribiría ninguna edición ni valor ya equivalente.":"No hay movimientos actuales que coincidan. Sí se aplicará a movimientos nuevos que cumplan las condiciones."));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido previsualizar"));}finally{setLoading(false);}}
  async function saveRule(){if(!editor)return;const error=validate(editor);if(error){setFeedback(failure(error));return;}setLoading(true);setFeedback(null);try{const j=await post({kind:"save",id:editor.id||null,rule:payload(editor)});if(j.overview)setData(j.overview);else await reload();setEditor(null);setPreview(null);setFeedback(success(editor.id?"Regla actualizada.":"Regla creada. Ya se aplicará automáticamente a los movimientos nuevos que coincidan."));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido guardar la regla"));}finally{setLoading(false);}}
  async function applyExisting(rule:TransactionRule){if(!rule.active)return;if(!window.confirm(`¿Aplicar “${rule.name}” a los movimientos existentes que coincidan? Solo escribirá en la capa editable y respetará campos editados manualmente.`))return;setLoading(true);setFeedback(null);try{const j=await post({kind:"apply",id:rule.id});if(j.overview)setData(j.overview);else await reload();setFeedback(success(`Regla aplicada: ${j.applied??0} movimientos modificados de ${j.matched??0} coincidencias${j.capped?" · límite de 5.000 alcanzado":""}.`));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido aplicar la regla"));}finally{setLoading(false);}}
  async function deactivate(rule:TransactionRule){if(!window.confirm(`¿Desactivar “${rule.name}”? Dejará de actuar sobre movimientos nuevos, pero sus cambios anteriores permanecerán hasta que uses “Deshacer”.`))return;setLoading(true);setFeedback(null);try{const j=await post({kind:"deactivate",id:rule.id});if(j.overview)setData(j.overview);else await reload();setFeedback(success("Regla desactivada."));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido desactivar"));}finally{setLoading(false);}}
  async function reactivate(rule:TransactionRule){const next={...editorFromRule(rule),active:true};setLoading(true);setFeedback(null);try{const j=await post({kind:"save",id:rule.id,rule:payload(next)});if(j.overview)setData(j.overview);else await reload();setFeedback(success("Regla reactivada."));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido reactivar"));}finally{setLoading(false);}}
  async function revert(rule:TransactionRule){if(!window.confirm(`¿Deshacer las aplicaciones activas de “${rule.name}”? Los campos modificados manualmente después de la regla se conservarán.`))return;setLoading(true);setFeedback(null);try{const j=await post({kind:"revert",id:rule.id});if(j.overview)setData(j.overview);else await reload();setFeedback(success(`Deshacer completado: ${j.applicationsReverted??0} aplicaciones revisadas y ${j.fieldsReverted??0} campos revertidos de forma segura.`));}catch(e){setFeedback(failure(e instanceof Error?e.message:"No se ha podido deshacer"));}finally{setLoading(false);}}
  ...
  return <div className={`rules-module ${loading?"is-loading":""}`} aria-busy={loading?"true":undefined}>
    ...
    <section className="rules-panel"><div className="rules-panel-head"><div><p className="eyebrow">AUTOMATIZACIÓN PRIVADA</p><h2>Reglas configuradas</h2></div><span className="pill">Prioridad menor = se evalúa antes</span></div>
      ...
      <div className="rules-list">{data.rules.map(rule=><article className={`rule-card ${rule.active?"active":"inactive"}`} key={rule.id}>
        <div className="rule-card-head"><div><div className="rule-meta"><span className={`status-badge ${rule.active?"ok":"muted"}`}>{rule.active?"Activa":"Inactiva"}</span><span>Prioridad {rule.priority}</span>{rule.stopProcessing&&<span>Detiene reglas posteriores</span>}</div><h3>{rule.name}</h3></div><div className="rule-card-count"><strong>{rule.activeApplicationCount}</strong><span>aplicaciones vigentes</span></div></div>
    ...
    {editor&&<div className="rules-modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget&&!loading)setEditor(null)}}><section className="rules-modal" role="dialog" aria-modal="true" aria-labelledby="rule-editor-title" aria-busy={loading?"true":undefined}>
