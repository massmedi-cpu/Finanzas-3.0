...
export function validateAccountUniqueness(
  ...
  return duplicated
    ? [{ field: "name", code: "duplicate_account_name", message: "Ya existe una cuenta con ese nombre." }]
    : [];
}
...
export function validateCategoryUniqueness(
  ...
  return duplicated
    ? [{ field: "name", code: "duplicate_category_name", message: "Ya existe una categoría con ese nombre en este nivel." }]
    : [];
}
...
export function validateCategoryHierarchy(
  ...
  const activeChild = categories.some(
    (category) => category.parentCategoryId === candidate.id && category.lifecycle === "active",
  );
  ...
  const incompatibleChild = categories.some(
    (category) => category.parentCategoryId === candidate.id && category.kind !== candidate.kind,
  );
  ...
  if (!candidate.parentCategoryId) return [];

  if (candidate.parentCategoryId === candidate.id) {
    return [{ field: "parentCategoryId", code: "self_parent", message: "Una categoría no puede depender de sí misma." }];
  }

  const byId = new Map(categories.map((category) => [category.id, category]));
  const parent = byId.get(candidate.parentCategoryId);
  if (!parent) {
    return [{ field: "parentCategoryId", code: "parent_not_found", message: "La categoría superior no existe." }];
  }
  ...
  if (parent.parentCategoryId) {
    return [{
      field: "parentCategoryId",
      code: "category_depth_limit",
      message: "Financial App admite una categoría principal y un único nivel de subcategoría.",
    }];
  }
...
export function validateAccountReorder(
  ...
  if (setIssues.length > 0) return setIssues;
...
export function validateCategoryReorder(
  ...
  const crossesCategoryBoundary = orderedIds.some((id, index) => {
    ...
    const incoming = byId.get(id);
    return !current || !incoming || current.kind !== incoming.kind || current.parentCategoryId !== incoming.parentCategoryId;
  });
