...
const versionSource=fs.readFileSync("lib/app-version.ts","utf8");
const expectedVersion=versionSource.match(/APP_VERSION\s*=\s*["']([^"']+)/)?.[1]||"";

if(!expectedVersion)throw new Error("APP_VERSION could not be resolved");
...
const headers={accept:"application/json","user-agent":`Financial-App-authenticated-preview-e2e/${expectedVersion}`};
...
if(payload.ok!==true||payload.privateSession!==true||payload.version!==expectedVersion)throw new Error("Probe identity/version contract failed");
const expectedChecks=["accountsReadable","archiveReadable","forecastContracts","forecastReadable","homePulseReadable","intelligenceContracts","intelligenceReadable","matchingObservabilityReadable","matchingQualityGate","movementsReadable"];
...
console.log(`Authenticated preview E2E OK · ${expectedVersion} · private accounts, home pulse, movements, forecast, archive, matching-quality and actionable intelligence verified · token replay rejected`);
