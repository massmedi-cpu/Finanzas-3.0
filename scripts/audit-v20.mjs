...
const navigation=existsSync("components/app-navigation.tsx")?readFileSync("components/app-navigation.tsx","utf8"):"";
...
if(!navigation.includes('["Plan","/plan"]'))errors.push("Plan no está en la navegación primaria de producto");
const mobileBlock=navigation.match(/const mobilePrimary = \[[\s\S]*?\] as const;/)?.[0]||"";
if(!mobileBlock.includes('["Plan","/plan"]'))errors.push("Plan no está en navegación móvil principal");
...
console.log("Financial App 2.0 audit OK · plan unificado, una RPC, trazabilidad, seguridad, responsive y navegación actual protegidos");
