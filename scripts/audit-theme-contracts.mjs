...
for(const token of [
  ...
  "--background-primary:","--surface-primary:","--text-primary:","--accent-primary:","--positive:","--negative:","--warning:","--info:","--focus:"
]){if(!globals.includes(token))failures.push(`Falta contrato visual de tema: ${token}`);}
for(const forbidden of ["#f4f1e9","#eee9df","#8a641b","#d7b35c","#f0e4c8"]){if(globals.includes(forbidden))failures.push(`Permanece identidad crema/dorada heredada en la paleta global: ${forbidden}`);}
...
const publicShell=chrome.slice(chrome.indexOf("if(publicRoute)"),chrome.indexOf('return <div className="app-root private">'));
if(!publicShell.includes("<ThemeController/>")||!publicShell.includes("{children}"))failures.push("Login/auth deben mantener activo ThemeController para no perder la preferencia al entrar o salir");
...
for(const token of ["THEME_STORAGE_KEY","THEME_CHROME","root.dataset.theme=effective","root.dataset.themePreference=preference","root.style.colorScheme=effective","meta.content=THEME_CHROME[effective]",'light:"#f4f7fb"','dark:"#070d18"']){if(!theme.includes(token))failures.push(`Runtime canónico de tema incompleto: ${token}`);}
for(const token of ["background:var(--accent-primary)","color:var(--negative)","color:var(--positive)"]){if(!controls.includes(token))failures.push(`Los controles semánticos deben consumir tokens centrales: ${token}`);}
...
console.log("Theme contract audit OK · sistema claro/oscuro, azul financiero premium y colores semánticos protegidos");
