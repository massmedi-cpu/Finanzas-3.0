...
for(const token of ["AUTO_SYNC_COOLDOWN_MS=15*60*1000","AUTO_SYNC_STALE_MS=30*60*1000","financial-app-auto-sync-at","router.refresh()","autoSync","const freshnessAnchor=lastSyncAt||sourceModifiedAt","ageMs(freshnessAnchor)>=AUTO_SYNC_STALE_MS"]){if(!syncButton.includes(token))errors.push(`Falta garantía de actualización inteligente: ${token}`);}
if(syncButton.includes("ageMs(lastSyncAt)>=AUTO_SYNC_STALE_MS||ageMs(sourceModifiedAt)>=AUTO_SYNC_STALE_MS"))errors.push("La antigüedad del archivo fuente no puede forzar sync si existe una sincronización reciente");
...
console.log("Financial App sync performance audit OK · incremental + auto refresh con frescura anclada al último sync + salud documental unificada");
