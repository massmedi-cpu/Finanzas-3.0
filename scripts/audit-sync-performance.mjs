...
for(const token of requiredEdge){
  if(!edge.includes(token)) errors.push(`Falta contrato incremental/telemetría de sync: ${token}`);
}

if(!/Promise\.all\(\[driveMeta\(token\),\s*sourceState\(\),\s*driveSyncState\(\)\]\)/.test(edge)){
  errors.push("Meta de Drive, fuente bancaria y estado incremental deben resolverse en paralelo");
}
if(!/driveDocumentDelta\(token,\s*String\(documentState\?\.changeToken\s*\|\|\s*""\),\s*driveStats\)/.test(edge)){
  errors.push("El escaneo documental debe partir del changeToken persistido");
}
if(!edge.includes('const nextToken = await driveStartPageToken(token, stats);')||!edge.includes('const files = await driveDocumentsFull(token, stats);')){
  errors.push("Debe conservarse full scan seguro para inicialización/recuperación");
}
...
if(snapshotIndex>=0&&finalizeIndex>=0&&finalizeIndex<snapshotIndex){
  errors.push("El autoenlace no puede ejecutarse antes de aplicar los movimientos nuevos");
}
...
if(performanceMigration.includes("for v_item in select value from jsonb_array_elements(p_files) loop")){
  errors.push("No puede reaparecer el bucle SQL documento-a-documento");
}
...
for(const token of [
  ...
  "change_token=coalesce(excluded.change_token",
  ...
  if(!incrementalMigration.includes(token)) errors.push(`Falta garantía SQL incremental: ${token}`);
}
...
console.log("Financial App sync performance audit OK · change feed incremental, delta atómico, fallback full scan, orden y telemetría protegidos");
