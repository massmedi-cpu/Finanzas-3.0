...
const version=appVersion.match(/APP_VERSION\s*=\s*"([^"]+)"/)?.[1]||"0.0.0";
const parts=value=>value.split(".").map(Number);
const atLeast=(value,min)=>{const a=parts(value),b=parts(min);for(let i=0;i<3;i++){if((a[i]||0)!==(b[i]||0))return(a[i]||0)>(b[i]||0)}return true};

must(atLeast(version,"3.6.0"),"La versión visible no puede retroceder por debajo de 3.6.0");
...
for(const token of ["financial_app_forecast_calendar","previous_year_seasonal","annualInsuranceAndTaxPatterns","actualMovementConfirms","interval_months=12","'received'"])
  must(migration.includes(token),`Motor de calendario 3.6 incompleto: ${token}`);
must(migration.includes("revoke all on function financial_app.forecast_calendar_core(date,integer) from public,anon")&&migration.includes("grant execute on function public.financial_app_forecast_calendar(date,integer) to authenticated,service_role"),"El RPC base debe mantener autorización explícita");
...
console.log(`Forecast calendar 3.6 audit OK · garantías base preservadas en producto ${version}`);
