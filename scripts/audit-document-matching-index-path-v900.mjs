...
const candidatesFile="database/FINANCIAL_APP_9.0.0_DOCUMENT_MATCHING_INDEX_PATH.sql";
const existenceFile="database/FINANCIAL_APP_9.0.0_DOCUMENT_MATCH_EXISTENCE_INDEX_PATH.sql";
const sql=fs.readFileSync(candidatesFile,"utf8");
const existence=fs.readFileSync(existenceFile,"utf8");
const lower=sql.toLowerCase();
const existenceLower=existence.toLowerCase();
const failures=[];
...
for(const token of [
  "create or replace function financial_app.document_has_match_candidate_core",
...
])must(existenceLower.includes(token.toLowerCase()),`Existencia de candidato indexada incompleta: ${token}`);

must((existenceLower.match(/join financial_app\.transactions t/g)||[]).length===2,"La existencia debe tener una ruta física por modo");
must((existenceLower.match(/abs\(t\.source_amount\) between/g)||[]).length===2,"La existencia debe usar rango indexable en ambos modos");
must((existenceLower.match(/limit 1/g)||[]).length===2,"Cada ruta de existencia debe detenerse en el primer candidato");
for(const forbidden of [
  "insert into financial_app.transaction_documents",
  "update financial_app.transactions",
  "delete from financial_app.transactions",
  ...
  "create or replace function public.",
  "grant execute"
])must(!existenceLower.includes(forbidden),`La optimización de existencia no puede contener: ${forbidden}`);
...
console.log("Document matching index-path audit OK · candidatos y existencia usan rutas standard/installment indexables, scoring/política intactos y cero mutación");
