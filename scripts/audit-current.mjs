...
const sharedControlSelectors=new Set(["button",".primary-action",".ghost",".danger-action",".icon-button",".button-link","button:disabled","[aria-disabled=\"true\"]","button[aria-busy=\"true\"]","a[aria-busy=\"true\"]",".inline-alert",".inline-alert.success",".inline-alert.error",".empty-state",".empty-state strong"]);
...
for(const file of cssFiles){const selectors=cssSelectors(read(file));if(file!=="app/controls.css"){for(const selector of selectors)must(!sharedControlSelectors.has(selector),`Selector compartido ${selector} redefinido fuera de controls.css: ${file}`);for(const selector of selectors)must(!/^\.(?:primary-action|ghost|danger-action|icon-button|button-link|inline-alert|empty-state)(?=[:[])/.test(selector),`Estado de control compartido redefinido sin ámbito fuera de controls.css: ${selector} en ${file}`);}}
const controls=read("app/controls.css");
for(const token of ["button{font:inherit}",".primary-action{","background:var(--gold-primary)",".ghost{",".danger-action{",".icon-button{",".button-link{display:inline-flex",".inline-alert{",".empty-state{","button:disabled","button[aria-busy=\"true\"]",".primary-action[aria-busy=\"true\"]::before"])must(controls.includes(token),`controls.css ha perdido la garantía compartida: ${token}`);
...
for(const forbidden of ["cash-flow.css","explainability.css","integrity.css","analysis.css","plan.css","budget.css","movements.css"])must(!rootLayout.includes(forbidden),`El layout raíz carga CSS específico de módulo: ${forbidden}`);
