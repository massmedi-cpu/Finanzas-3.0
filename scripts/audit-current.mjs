...
for(const token of ["button{font:inherit}",".primary-action{","background:var(--accent-primary)",".ghost{",".danger-action{",".icon-button{",".button-link{display:inline-flex",".inline-alert{",".empty-state{","button:disabled","button[aria-busy=\"true\"]",".primary-action[aria-busy=\"true\"]::before"])must(controls.includes(token),`controls.css ha perdido la garantía compartida: ${token}`);
