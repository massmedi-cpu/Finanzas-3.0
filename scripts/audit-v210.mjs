...
const errors=[];
const need=["components/intent-link.tsx","components/app-sidebar.tsx","app/loading.tsx","app/readability-v210.css","docs/RELEASE_GATE_V2.1.0.md","docs/PERFORMANCE_V2.1.0.md","docs/COHERENCE_V2.1.0.md","docs/legacy/RELEASE_GATE_V2.1.0_PRE_REBUILD.md"];
...
if(!errors.length){
  ...
  const movementsClient=readFileSync("app/movimientos/movements-client.tsx","utf8");
  const movementsApi=readFileSync("app/api/movements/route.ts","utf8");
  const performance=readFileSync("docs/PERFORMANCE_V2.1.0.md","utf8");
  const coherence=readFileSync("docs/COHERENCE_V2.1.0.md","utf8");
  const planLib=readFileSync("lib/financial/plan.ts","utf8");
  const homePage=readFileSync("app/page.tsx","utf8");
  const planPage=readFileSync("app/plan/page.tsx","utf8");
  ...
  if(!gate.includes("CANDIDATA VALIDADA")||!gate.includes("2.0.1 permanece congelada"))errors.push("El gate 2.1 no identifica la candidata validada o no protege el checkpoint 2.0.1");
...
console.log("Financial App 2.1 audit OK · navegación por intención, legibilidad, shell único, Movimientos ligero, Plan coherente y checkpoint protegido");
