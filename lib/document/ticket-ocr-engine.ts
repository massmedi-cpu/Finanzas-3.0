...
export function mergeReceiptTexts(primaryText:string,alternateText:string){
  const primary=normalizeOcrText(primaryText).split(/\r?\n/).filter(Boolean);const alternate=normalizeOcrText(alternateText).split(/\r?\n/).filter(Boolean);if(!primary.length)return normalizeOcrText(alternateText);if(!alternate.length)return normalizeOcrText(primaryText);const used=new Set<number>();
...
export async function recognizeTicketImage(file:File,worker:Worker,onProgress:(value:number,label:string)=>void,hint:DocumentTypeHint=null):Promise<ImageOcrResult>{
  ...
  try{
    ...
    onProgress(.97,"Validando comercio, fecha, líneas e importe");return{text:merged,layoutText:best.layoutText,confidence:best.confidence,method:`image_ocr_receipt_v501:fastcrop_v2:${best.variant}`,passes:candidates.map(({variant,confidence,score})=>({variant,confidence,score:Math.round(score*10)/10})),receiptLayout:layout,metadata:{documentType:"receipt",documentDate,amount,merchant,lines:normalizeOcrText(merged).split(/\r?\n/).filter(Boolean)}};
