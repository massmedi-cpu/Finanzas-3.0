...
export function parseTsvWords(tsv:string){
  ...
  for(const row of String(tsv||"").replace(/\r/g,"").split("\n").slice(1)){
    ...
    if(!text||!Number.isFinite(conf)||conf<18||![left,top,width,height].every(Number.isFinite)||width<=0||height<=0)continue;
...
function headerAnchors(line:TsvLine){
  ...
  for(const word of words){const key=normalizeKey(word.text);if(!qty&&/^(UDS?|UND|UNDS|CANT|CANTIDAD)$/.test(key))qty=word;if(!price&&/^PRECI/.test(key))price=word;if(!total&&/^(IMPORTE|TOTAL)$/.test(key))total=word;}
  if(!qty||!price)return null;
  const qtyX=center(qty);const priceX=center(price);let totalX=total?center(total):priceX+Math.max(70,(priceX-qtyX)*.95);
  if(!(qtyX<priceX&&priceX<totalX))return null;
...
export function parseReceiptTsvLayout(tsv:string):ReceiptLayout|null{
  ...
  const headerIndex=lines.findIndex(line=>{const key=normalizeKey(line.plain);return key.includes("DESCRIP")&&(/UDS|UND|CANT/.test(key))&&key.includes("PRECI");});
  ...
  for(const line of lines.slice(headerIndex+1)){
    ...
    const summaryMatch=plain.match(summaryRegex);
    ...
    let description=lineText(descriptionWords);let quantity=numericFromWords(qtyWords,"qty");let unitPrice=numericFromWords(priceWords,"money");let total=numericFromWords(totalWords,"money");
    if(!description||!quantity||!unitPrice||!total){
      ...
      if(decimals.length>=2&&qtyCandidate){unitPrice=unitPrice||decimals.at(-2)!;total=total||decimals.at(-1)!;quantity=quantity||qtyCandidate;const numericStart=plain.indexOf(qtyCandidate);if(!description&&numericStart>0)description=cleanLine(plain.slice(0,numericStart));}
    }
    if(description&&quantity&&unitPrice&&total&&plausibleItem(quantity,unitPrice,total))items.push({description,quantity:receiptDisplayNumber(quantity),unitPrice:receiptDisplayNumber(unitPrice),total:receiptDisplayNumber(total)});
  }
