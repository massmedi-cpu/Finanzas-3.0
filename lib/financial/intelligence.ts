...
function confidence(plan:PlanOverview):{level:IntelligenceConfidence;reasons:string[]}{
  ...
  if(c.duplicates>0)reasons.push(`${c.duplicates} posibles duplicados`);
  if(c.closeBlockers>0)reasons.push(`${c.closeBlockers} bloqueos de cierre`);
  if(!coverage.currentComplete)reasons.push(`patrimonio con cobertura ${coverage.knownAccounts}/${coverage.accountCount}`);
  if(c.needsReview>0)reasons.push(`${c.needsReview} movimientos por revisar`);
  if(c.unreconciled>0)reasons.push(`${c.unreconciled} movimientos sin conciliar`);
  if(c.unbudgetedSpent>0)reasons.push(`${money.format(c.unbudgetedSpent)} sin presupuesto`);
...
export function buildDecisionIntelligence(plan:PlanOverview,analytics?:AnalysisInsights|null):DecisionIntelligence{
  ...
  if(s.monthlyNet<0&&s.forecastProjectedNet90<0)add({key:"deficit-persists",severity:"high",kind:"risk",title:"El déficit del mes también aparece a 90 días",detail:`El mes va en ${money.format(s.monthlyNet)} y la previsión acumula ${money.format(s.forecastProjectedNet90)}.`,href:d.forecast.href,sourcePath:"summary.monthlyNet + summary.forecastProjectedNet90"});
  if(s.budgetProjectedDifference!=null&&s.budgetProjectedDifference<0)add({key:"budget-projection-negative",severity:"high",kind:"risk",title:"El presupuesto proyecta exceso",detail:`Manteniendo el ritmo actual, la diferencia proyectada es ${money.format(s.budgetProjectedDifference)}.`,href:d.budget.href,sourcePath:"summary.budgetProjectedDifference"});
  if(s.capacityAfterGoals<0)add({key:"goals-over-capacity",severity:"high",kind:"risk",title:"Los objetivos exigen más que tu capacidad de referencia",detail:`Faltan ${money.format(Math.abs(s.capacityAfterGoals))} al mes para cubrir el esfuerzo requerido con la capacidad calculada.`,href:d.goals.href,sourcePath:"summary.capacityAfterGoals"});
  if(c.closeBlockers>0)add({key:"close-blockers",severity:"high",kind:"quality",title:"El cierre mensual sigue bloqueado",detail:`Hay ${c.closeBlockers} condiciones que deben resolverse antes de considerar el mes cerrado.`,href:d.control.href,sourcePath:"domains.control.snapshot.closeBlockers"});
  if(!d.netWorth.coverage.currentComplete)add({key:"net-worth-coverage",severity:"medium",kind:"quality",title:"El patrimonio no tiene cobertura completa",detail:`Hay saldo conocido en ${d.netWorth.coverage.knownAccounts} de ${d.netWorth.coverage.accountCount} cuentas.`,href:d.netWorth.href,sourcePath:"domains.netWorth.coverage"});
  if(analytics?.recentExpenseTrendPercent!=null){const trend=analytics.recentExpenseTrendPercent;add({key:"expense-trend",severity:trend>0&&s.monthlyNet<0?"medium":"low",kind:"context",title:trend>0?"El gasto reciente está subiendo":"El gasto reciente está bajando",detail:`La media del último bloque de meses completos cambia ${trend>0?"+":""}${pct.format(trend)} % frente al bloque comparable anterior.`,href:"/analisis",sourcePath:"analysis.recentExpenseTrendPercent"})}
  if(!signals.some(signal=>signal.severity==="critical"||signal.severity==="high")&&s.capacityAfterGoals>0&&s.monthlyNet>0&&d.forecast.lowestBalance>=0)add({key:"available-capacity",severity:"low",kind:"opportunity",title:"Hay margen positivo después de objetivos",detail:`La capacidad de referencia deja ${money.format(s.capacityAfterGoals)} al mes tras cubrir el esfuerzo de objetivos.`,href:"/plan",sourcePath:"summary.capacityAfterGoals"});
