...
export type ForecastOverview = {
  version:string; startDate:string; endDate:string; days:number; currentBalance:number; projectedBalance:number; projectedIncome:number; projectedExpenses:number; projectedNet:number; lowestBalance:number;
  ...
  rules:{ automaticSuggestionsAreReadOnly:boolean; historyWindowDays:number; amountTolerancePercent:number; dateToleranceDays:number; stalePatternsExcluded:boolean };
};
...
export async function getForecastOverview(days=90):Promise<ForecastOverview>{
  ...
  const r=data as any;
  return {
    version:String(r.version||"0.7.0"),startDate:String(r.startDate),endDate:String(r.endDate),days:n(r.days),currentBalance:n(r.currentBalance),projectedBalance:n(r.projectedBalance),projectedIncome:n(r.projectedIncome),projectedExpenses:n(r.projectedExpenses),projectedNet:n(r.projectedNet),lowestBalance:n(r.lowestBalance),
    ...
    consolidatedCount:n(r.consolidatedCount),rules:r.rules||{automaticSuggestionsAreReadOnly:true,historyWindowDays:270,amountTolerancePercent:12,dateToleranceDays:5,stalePatternsExcluded:true},
  };
}
