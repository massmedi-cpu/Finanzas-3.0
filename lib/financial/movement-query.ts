...
function tri(value: string | undefined): TriFilter {
...
function amount(value: string | undefined) {
  ...
  return Number.isFinite(Number(normalized.replace(",", "."))) ? normalized : "";
}
...
export function parseMovementSearchParams(params: Record<string, string | undefined>): MovementFilterState {
  ...
  return {
    ...
    account: text(params.account),
    type: text(params.type),
    category: text(params.category),
    subcategory: text(params.subcategory),
    merchant: text(params.merchant),
    channel: text(params.channel),
    tag: text(params.tag),
    review: flag(params.review),
    cashFlowOnly: flag(params.cashFlow),
    recurring: tri(params.recurring),
    internalTransfer: tri(params.internalTransfer),
    reconciled: tri(params.reconciled),
    documents: tri(params.documents),
    splits: tri(params.splits),
    from: date(params.from),
    to: date(params.to),
    min: amount(params.min),
    max: amount(params.max),
    sort: SORTS.has(requestedSort) ? requestedSort : "date_desc",
  };
}
...
export function movementFiltersForData(state: MovementFilterState): MovementFilters {
  return {
    ...
    tag: state.tag || null,
    reviewOnly: state.review,
    cashFlowOnly: state.cashFlowOnly,
    recurring: bool(state.recurring),
    internalTransfer: bool(state.internalTransfer),
    reconciled: bool(state.reconciled),
    hasDocuments: bool(state.documents),
    hasSplits: bool(state.splits),
    dateFrom: state.from || null,
    dateTo: state.to || null,
    minAmount: numeric(state.min),
    maxAmount: numeric(state.max),
    sort: state.sort,
  };
}
...
export function movementSearchParams(state: MovementFilterState) {
  ...
  if (state.subcategory) q.set("subcategory", state.subcategory);
  if (state.merchant) q.set("merchant", state.merchant);
  if (state.channel) q.set("channel", state.channel);
  if (state.tag) q.set("tag", state.tag);
  if (state.review) q.set("review", "1");
  if (state.cashFlowOnly) q.set("cashFlow", "1");
  if (state.recurring) q.set("recurring", state.recurring);
  if (state.internalTransfer) q.set("internalTransfer", state.internalTransfer);
  if (state.reconciled) q.set("reconciled", state.reconciled);
  if (state.documents) q.set("documents", state.documents);
  if (state.splits) q.set("splits", state.splits);
...
export function movementUrl(state: MovementFilterState) {
  const query = movementSearchParams(state).toString();
  return query ? `/movimientos?${query}` : "/movimientos";
}

export function movementState(overrides: Partial<MovementFilterState> = {}): MovementFilterState {
...
export function bucketBounds(start: string, bucket: MovementBucket, minDate?: string, maxDate?: string) {
  const parsed = new Date(`${start}T00:00:00Z`);
  if (Number.isNaN(parsed.getTime())) return { from: minDate || start, to: maxDate || start };
