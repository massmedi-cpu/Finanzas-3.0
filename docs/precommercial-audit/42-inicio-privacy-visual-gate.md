...
- `d24544651e19caac2bfc36986df8208bd7bc5ca9`: preflight final con 10.0.7 sincronizada, workflow temporal eliminado y contrato de versión incorporado al gate permanente. El movimiento de ref por API tampoco generó un evento de Actions, por lo que no se toma como certificación.

## Gate final

El commit documental posterior a este preflight fuerza el evento `push` normal y será el candidato exacto que debe superar de nuevo build + Playwright desktop/móvil, incluido `release-version-contract.spec.ts`. El job protegido de Vercel no se fuerza mientras exista `build-rate-limit`. 10.0.7 permanecerá guardada en rama/PR y **no se fusionará a `main` antes de cerrar Production 10.0.6**, para impedir que el siguiente deployment salte directamente desde la Production real 10.0.5 a 10.0.7.
