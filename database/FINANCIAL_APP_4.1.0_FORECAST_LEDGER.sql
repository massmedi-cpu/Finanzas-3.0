...
-- Financial App 4.1.0 — Previsión mensual verificable, reversible y con conciliación 1↔1.

create or replace function financial_app.forecast_norm(p_value text)
returns text
language sql
immutable
set search_path=pg_catalog
as $$
  select trim(regexp_replace(regexp_replace(lower(coalesce(p_value,'')),'[^[:alnum:]]+',' ','g'),'[[:space:]]+',' ','g'))
$$;
revoke all on function financial_app.forecast_norm(text) from public,anon,authenticated;

create or replace function financial_app.forecast_is_annual_signal(p_category text,p_subcategory text,p_title text)
returns boolean
language sql
immutable
set search_path=pg_catalog
as $$
  select lower(coalesce(p_category,'')||' '||coalesce(p_subcategory,'')) ~ '(seguros|impuestos y tasas|seguro|impuesto|tasa municipal|ibi|ivtm|tribut)'
      or lower(coalesce(p_title,'')) ~ '(seguro|línea directa|linea directa|domiciliacion impuesto|domiciliación impuesto|impuesto:|\mibi\M|\mivtm\M|tributo|tasa municipal)'
$$;
revoke all on function financial_app.forecast_is_annual_signal(text,text,text) from public,anon,authenticated;
...
create or replace function financial_app.restore_forecast_event(p_event_id text)
returns boolean
language plpgsql
security definer
set search_path=pg_catalog,financial_app,auth
...
begin
  v_email:=financial_app.authorized_email();
  if v_email is null then raise exception 'forbidden' using errcode='42501'; end if;
  if nullif(trim(coalesce(p_event_id,'')),'') is null then raise exception 'invalid forecast event'; end if;
  delete from financial_app.forecast_event_overrides
  where user_email=v_email and event_id=trim(p_event_id) and action='dismissed';
  get diagnostics v_rows=row_count;
  return v_rows>0;
end
$$;
revoke all on function financial_app.restore_forecast_event(text) from public,anon;
grant execute on function financial_app.restore_forecast_event(text) to authenticated,service_role;

create or replace function public.financial_app_restore_forecast_event(p_event_id text)
returns boolean
language sql
set search_path=pg_catalog,financial_app
as $$ select financial_app.restore_forecast_event(p_event_id) $$;
revoke all on function public.financial_app_restore_forecast_event(text) from public,anon;
grant execute on function public.financial_app_restore_forecast_event(text) to authenticated,service_role;

insert into financial_app.app_meta(key,value,updated_at)
values
  ('app_version',to_jsonb('4.1.0'::text),now()),
  ('target_version',to_jsonb('4.1.0'::text),now())
