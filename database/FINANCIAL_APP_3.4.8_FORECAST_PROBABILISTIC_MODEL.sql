-- Financial App 3.4.8 — probabilistic forecast semantics and estimated dates
-- Historical patterns are predictions, never user-confirmed movements.
-- A prediction affects the estimated balance; only a real bank transaction makes it real.
...
begin
  ...
  with raw_base as(
    ...
    where a.account_role='operating' and t.source_missing=false and t.is_duplicate=false and t.is_internal_transfer=false
      ...
      and coalesce(t.effective_date,t.source_date)>=p_start-1460 and coalesce(t.effective_date,t.source_date)<=p_start
    ...
    select key_norm,amount_sign,d,(array_agg(label order by abs(amount) desc))[1] label,
      ...
      (array_agg(amount order by abs(amount) desc))[1] amount
    ...
    case when amount_sign>0 and n=1 and last_date>=p_start-45 and lower(coalesce(category,'')) like 'ingresos laborales%' and median_abs>=400
      then 'Ingreso laboral reciente; posible repetición mensual'
      else 'Patrón recurrente detectado en tu historial' end reason
    from stats
  ),patterns as(
    select *,least(.97,greatest(.45,case when n=1 then .45 else .52 end+least(n,10)*.035+(1-least(amount_cv,1))*.10+(case when interval_months=12 then .05 else .08 end)-(least(coalesce(interval_sd,0),40)/40)*.05)) confidence
    from classified c
    where interval_months is not null
      and not exists(
        select 1 from financial_app.forecasts f
        where f.status<>'cancelled'::financial_app.forecast_status
          and(lower(coalesce(f.counterparty,''))=c.key_norm or lower(f.title)=c.key_norm)
      )
      and not (c.amount_sign>0 and lower(coalesce(c.category,'')) like 'ingresos laborales%' and exists(
        select 1 from classified newer
        where newer.amount_sign>0 and lower(coalesce(newer.category,'')) like 'ingresos laborales%'
          and newer.key_norm<>c.key_norm and newer.last_date>c.last_date and newer.last_date>=p_start-45
      ))
  ),targets as(
    select p.*,g.step,(p.last_date+make_interval(months=>p.interval_months*g.step))::date raw_target
    from patterns p cross join generate_series(1,48) g(step)
  ),occurrences as(
    select t.*,
      make_date(
        extract(year from raw_target)::int,
        extract(month from raw_target)::int,
        least(
          greatest(1,round(median_day)::int),
          extract(day from (date_trunc('month',raw_target)+interval '1 month - 1 day'))::int
        )
      ) next_date
    from targets t
  ),future_occurrences as(
    select * from occurrences where next_date between p_start and v_end
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'explanation',jsonb_build_object(
      'observations',n,
      'medianIntervalDays',case when median_gap is null then null else round(median_gap::numeric,1) end,
      'amountVariation',round((amount_cv*100)::numeric,1),
      'intervalVariationDays',round(coalesce(interval_sd,0)::numeric,1),
      'typicalDayOfMonth',round(median_day::numeric,0),
      'dateVariationDays',round(coalesce(day_sd,0)::numeric,1),
      'source','historical_pattern_v3','reason',reason
    )
  ) order by next_date,confidence desc,abs(median_abs) desc),'[]'::jsonb)
  into v_suggestions from future_occurrences;

  select coalesce(jsonb_agg(jsonb_build_object(
    'id',f.id,'title',f.title,'date',f.predicted_date,'amount',f.predicted_amount,'category',f.category,'subcategory',f.subcategory,
    'counterparty',f.counterparty,'recurrence',f.recurrence_rule,'confidence',f.confidence,'notes',f.notes,'status',f.status
  ) order by f.predicted_date,f.title),'[]'::jsonb)
  into v_saved
  ...
  with stored as(
    select o.id::text id,o.occurrence_date event_date,o.predicted_amount amount,f.title,f.category,f.subcategory,f.counterparty,
      'saved'::text source,coalesce(f.confidence,1) confidence
    from financial_app.forecast_occurrences o
    join financial_app.forecasts f on f.id=o.forecast_id
    where o.status='pending' and o.occurrence_date between p_start and v_end and f.status<>'cancelled'::financial_app.forecast_status
  ),suggested as(
    select s.id,s."nextDate" event_date,s.amount,s.title,s.category,s.subcategory,s.counterparty,'suggested'::text source,s.confidence
    from jsonb_to_recordset(v_suggestions)
      as s(id text,title text,"nextDate" date,amount numeric,category text,subcategory text,counterparty text,confidence numeric)
  ),all_events as(
    select * from stored
    union all
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id',id,'date',event_date,'amount',amount,'title',title,'category',category,'subcategory',subcategory,'counterparty',counterparty,
    'source',source,'confidence',confidence
  ) order by event_date,id),'[]'::jsonb),
  coalesce(sum(amount) filter(where amount>0),0),
  coalesce(abs(sum(amount) filter(where amount<0)),0),
  coalesce(sum(amount),0)
  into v_events,v_income,v_expenses,v_net
  from all_events;

  v_projected:=v_balance+v_net;

  with e as(
    select * from jsonb_to_recordset(v_events) as x(id text,"date" date,amount numeric,title text,source text)
  ),o as(
    select id,"date" event_date,amount,title,source,sum(amount) over(order by "date",id) cum from e
  ),points as(
    select 0 seq,p_start point_date,v_balance balance,'Saldo actual' title,'current' source
    union all
    select row_number() over(order by event_date,id)::int,event_date,v_balance+cum,title,source from o
  )
  select coalesce(min(balance),v_balance),
    coalesce(jsonb_agg(jsonb_build_object('date',point_date,'balance',round(balance,2),'title',title,'source',source) order by seq),'[]'::jsonb)
  into v_lowest,v_series from points;

  with e as(
    select * from jsonb_to_recordset(v_events) as x("date" date,amount numeric)
  ),m as(
    select to_char(date_trunc('month',"date"),'YYYY-MM') month_key,
  ...
  select coalesce(jsonb_agg(jsonb_build_object('month',month_key,'income',income,'expenses',expenses,'net',net) order by month_key),'[]'::jsonb)
  into v_monthly from m;

  select count(*) into v_consolidated
  from financial_app.forecast_occurrences o
  join financial_app.forecasts f on f.id=o.forecast_id
  where o.status='consolidated' and o.occurrence_date between p_start and v_end and f.status<>'cancelled'::financial_app.forecast_status;
  ...
  return jsonb_build_object(
    ...
    'projectedBalance',round(v_projected,2),'projectedIncome',round(v_income,2),'projectedExpenses',round(v_expenses,2),
    'projectedNet',round(v_net,2),'lowestBalance',round(v_lowest,2),
    'suggestions',v_suggestions,'events',v_events,'balanceSeries',v_series,'monthly',v_monthly,'savedForecasts',v_saved,'consolidatedCount',v_consolidated,
    'rules',jsonb_build_object(
      'automaticSuggestionsAreReadOnly',true,
      'automaticSuggestionsRequireConfirmation',false,
      'suggestionsAffectProjection',true,
      'realOnlyAfterTransaction',true,
      'predictionDateUsesTypicalDay',true,
      'historyWindowDays',1460,
      'amountTolerancePercent',75,
      'dateToleranceDays',22,
      'stalePatternsExcluded',true,
      'futureSuggestedOccurrences',true
    )
  );
end;$function$;
...
begin
  ...
  v_overview:=financial_app.forecast_overview_core(p_start,p_days);
  v_balance:=coalesce((v_overview->>'currentBalance')::numeric,0);
  v_savings:=coalesce((v_overview->>'savingsBalance')::numeric,0);
  v_base_events:=coalesce(v_overview->'events','[]'::jsonb);
  v_base_series:=coalesce(v_overview->'balanceSeries','[]'::jsonb);
  v_base_net:=coalesce((v_overview->>'projectedNet')::numeric,0);
  v_base_projected:=coalesce((v_overview->>'projectedBalance')::numeric,v_balance+v_base_net);
  v_base_lowest:=coalesce((v_overview->>'lowestBalance')::numeric,v_balance);
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'id','scenario:'||n::text,'date',event_date,'amount',p_amount,'title',v_title,'source','scenario'
  ) order by event_date,n),'[]'::jsonb),
  ...
  with all_events as(
    ...
    select "date",sum(amount) amount,string_agg(title,' · ' order by id) title,
      bool_or(source='scenario') has_scenario,bool_or(source='suggested') has_suggested
    from all_events group by "date"
  ),o as(
    select "date",amount,title,has_scenario,has_suggested,sum(amount) over(order by "date") cum from daily
  ),points as(
    select 0 seq,p_start point_date,v_balance balance,'Saldo actual' title,'current' source
    union all
    select row_number() over(order by "date")::int,"date",v_balance+cum,title,
      case when has_scenario then 'scenario' when has_suggested then 'suggested' else 'saved' end
    from o
  )
  select coalesce(min(balance),v_balance),min(point_date) filter(where balance<0),
    coalesce(jsonb_agg(jsonb_build_object('date',point_date,'balance',round(balance,2),'title',title,'source',source) order by seq),'[]'::jsonb)
  ...
  return jsonb_build_object(
    ...
    'baseline',jsonb_build_object(
      'projectedBalance',round(v_base_projected,2),'net',round(v_base_net,2),'lowestBalance',round(v_base_lowest,2),
      'events',v_base_events,'balanceSeries',v_base_series
    ),
    'scenario',jsonb_build_object(
      'title',v_title,'amount',round(p_amount,2),'date',p_scenario_date,'frequency',v_frequency,'interval',v_interval,
      'requestedOccurrences',v_occurrences,'events',v_scenario_events,'delta',round(v_scenario_delta,2),
      'projectedBalance',round(v_scenario_projected,2),'lowestBalance',round(v_scenario_lowest,2),
      'firstNegativeDate',v_first_negative,'balanceSeries',v_scenario_series
    ),
    'rules',jsonb_build_object('readOnly',true,'savingsUsed',false,'suggestionsIncluded',true,'officialForecastsModified',false)
  );
end;$function$;

revoke all on function financial_app.forecast_overview_core(date,integer) from public,anon;
grant execute on function financial_app.forecast_overview_core(date,integer) to authenticated,service_role;
