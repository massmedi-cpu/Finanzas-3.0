...
begin
  ...
  v_forecast := financial_app.forecast_overview_core(current_date,90);
  ...
  v_lowest_balance := coalesce(nullif(v_forecast->>'lowestBalance','')::numeric,0);
  v_projected_net := coalesce(nullif(v_forecast->>'projectedNet','')::numeric,0);
  ...
  from jsonb_array_elements(coalesce(v_forecast->'balanceSeries','[]'::jsonb)) x
  where coalesce(nullif(x->>'balance','')::numeric,0) < 0
  ...
  return jsonb_build_object(
    ...
    'summary',jsonb_build_object(
      ...
      'forecastCurrentBalance',coalesce(nullif(v_forecast->>'currentBalance','')::numeric,0),'forecastProjectedBalance90',coalesce(nullif(v_forecast->>'projectedBalance','')::numeric,0),'forecastLowestBalance90',v_lowest_balance,'forecastProjectedNet90',v_projected_net,
    ...
    'domains',jsonb_build_object(
      ...
      'forecast',jsonb_build_object('currentBalance',v_forecast->'currentBalance','projectedBalance',v_forecast->'projectedBalance','projectedNet',v_forecast->'projectedNet','lowestBalance',v_forecast->'lowestBalance','firstNegativeDate',v_first_negative,'eventCount',jsonb_array_length(coalesce(v_forecast->'events','[]'::jsonb)),'suggestionCount',jsonb_array_length(coalesce(v_forecast->'suggestions','[]'::jsonb)),'href','/prevision'),
    ...
    'rules',jsonb_build_object('readOnlyDecisionLayer',true,'noAutomaticFinancialMutations',true,'forecastDays',90,'forecastSuggestionsAffectProjection',false,'goalCapacityMethod',v_goals->'capacityReferenceMethod','sourceFunctions',jsonb_build_array('budget_overview_core','forecast_overview_core','goals_overview_core','net_worth_overview_core','control_center_core'))
  );
end;
$$;
...
create or replace function public.financial_app_plan_overview(p_month date default current_date)
returns jsonb
language sql
set search_path = pg_catalog, financial_app, auth
as $$ select financial_app.plan_overview_core(p_month) $$;

revoke all on function public.financial_app_plan_overview(date) from public, anon;
grant execute on function public.financial_app_plan_overview(date) to authenticated, service_role;
