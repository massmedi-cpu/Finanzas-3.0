...
begin
  ...
  with raw_base as(
  ...
  ),seq as(select b.*,d-lag(d) over(partition by key_norm,amount_sign order by d) gap_days from daily b),stats as(
    ...
    case when amount_sign>0 and n=1 and last_date>=p_start-45 and lower(coalesce(category,'')) like 'ingresos laborales%' and median_abs>=400 then 'Ingreso laboral reciente; posible repetición mensual' else 'Patrón recurrente detectado en tu historial' end reason
    from stats
  ),patterns as(
    ...
    where interval_months is not null
      ...
      and not (c.amount_sign>0 and lower(coalesce(c.category,'')) like 'ingresos laborales%' and exists(
        select 1 from classified newer where newer.amount_sign>0 and lower(coalesce(newer.category,'')) like 'ingresos laborales%' and newer.key_norm<>c.key_norm and newer.last_date>c.last_date and newer.last_date>=p_start-45
      ))
  ),occurrences as(
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id',md5(key_norm||':'||amount_sign||':'||interval_months||':'||next_date),'title',label,'nextDate',next_date,'amount',round((median_abs*amount_sign)::numeric,2),'category',category,'subcategory',subcategory,'counterparty',counterparty,'confidence',round(confidence::numeric,2),'recurrence',jsonb_build_object('frequency',case when interval_months=12 then 'yearly' else 'monthly' end,'interval',case when interval_months=12 then 1 else interval_months end),'explanation',jsonb_build_object('observations',n,'medianIntervalDays',case when median_gap is null then null else round(median_gap::numeric,1) end,'amountVariation',round((amount_cv*100)::numeric,1),'intervalVariationDays',round(coalesce(interval_sd,0)::numeric,1),'source','historical_pattern_v2','reason',reason)) order by next_date,confidence desc,abs(median_abs) desc),'[]'::jsonb) into v_suggestions from occurrences;
  ...
  return jsonb_build_object('version','0.8.1','startDate',p_start,'endDate',v_end,'days',p_days,'currentBalance',round(v_balance,2),'savingsBalance',round(v_savings,2),'projectedBalance',round(v_projected,2),'projectedIncome',round(v_income,2),'projectedExpenses',round(v_expenses,2),'projectedNet',round(v_net,2),'lowestBalance',round(v_lowest,2),'suggestions',v_suggestions,'events',v_events,'balanceSeries',v_series,'monthly',v_monthly,'savedForecasts',v_saved,'consolidatedCount',v_consolidated,'rules',jsonb_build_object('automaticSuggestionsAreReadOnly',true,'suggestionsAffectProjection',false,'historyWindowDays',730,'amountTolerancePercent',75,'dateToleranceDays',22,'stalePatternsExcluded',true,'futureSuggestedOccurrences',true));
end;$function$;
