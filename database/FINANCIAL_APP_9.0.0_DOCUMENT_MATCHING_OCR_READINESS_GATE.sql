...
-- Financial App 9.0.0 — matching documental solo con evidencia financiera fiable.
-- Un OCR pendiente/degradado puede contener base, subtotal o importes parciales que
-- no son el total pagado. Esos valores no deben alimentar sugerencias, autoenlace,
-- observabilidad, operaciones ni forecast hasta que el OCR sea complete o manual.
-- Se preservan íntegros las rutas indexables, scoring, ranking y política existentes.
...
with policy as (
  ...
  select d.id,d.document_date,d.amount as document_amount,d.merchant,
    regexp_replace(lower(coalesce(d.merchant,'')),'[^a-z0-9áéíóúüñ]+','','g') as merchant_norm,
    coalesce(
      case when coalesce(d.ocr_data->>'chargeDate','') ~ '^\d{4}-\d{2}-\d{2}$'
        then (d.ocr_data->>'chargeDate')::date end,
      d.document_date
    ...
    case
      when coalesce(d.ocr_data->>'installmentCount','') ~ '^[0-9]+$'
        then greatest(1,least((d.ocr_data->>'installmentCount')::integer,60))
      else 1
  ...
  select
    p.id as document_id,p.merchant_norm,p.match_mode,p.target_amount,p.reference_date,p.document_date,p.payment_window,
    t.id as transaction_id,t.source_id,t.source_amount,
  ...
  select * from standard_tx
  union all
  select * from installment_tx
), enriched as (
  select c.*,
    regexp_replace(
      lower(coalesce(c.counterparty,'')||' '||coalesce(c.concept,'')),
      '[^a-z0-9áéíóúüñ]+','','g'
    ) as match_text
  from candidate_tx c
), base as (
  select
    e.transaction_id,e.source_id,e.match_date,e.source_amount as amount,e.concept,e.counterparty,e.match_mode,
    abs(abs(e.source_amount)-abs(e.target_amount))::numeric as amount_diff,
    abs(e.match_date-e.reference_date)::integer as days_diff,
    (length(e.merchant_norm)>=3 and e.match_text like '%'||e.merchant_norm||'%') as merchant_match,
    case when e.match_mode='installment'
      then case
        when abs(abs(e.source_amount)-abs(e.target_amount))<=0.02 then 60
        when abs(abs(e.source_amount)-abs(e.target_amount))<=0.50 then 48
        when abs(abs(e.source_amount)-abs(e.target_amount))<=greatest(1,abs(e.target_amount)*0.05) then 38
        else 20
      end
      else case
        when abs(abs(e.source_amount)-abs(e.target_amount))<=0.01 then 55
        when abs(abs(e.source_amount)-abs(e.target_amount))<=0.50 then 45
        when abs(abs(e.source_amount)-abs(e.target_amount))<=greatest(1,abs(e.target_amount)*0.05) then 35
        else 20
      end
    end as amount_score,
    case
      when e.match_mode='installment' then 20
      when abs(e.match_date-e.reference_date)=0 then 25
      when abs(e.match_date-e.reference_date)=1 then 22
      when abs(e.match_date-e.reference_date)<=3 then 18
      else 10
    end as date_score,
    case when length(e.merchant_norm)>=3 and e.match_text like '%'||e.merchant_norm||'%' then 20 else 0 end as merchant_score
  from enriched e
), scored as (
...
select
  transaction_id,source_id,match_date,amount,concept,counterparty,match_mode,
  round(score,1),confidence_tier,round(amount_diff,2),days_diff,merchant_match,
  candidate_rank_raw::integer,candidate_count_raw::integer,
  case when score_margin is null then null else round(score_margin,1) end,
  auto_eligible,reasons
from final
