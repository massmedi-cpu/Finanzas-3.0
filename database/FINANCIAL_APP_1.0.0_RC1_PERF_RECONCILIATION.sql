...
-- Rendimiento: calcula una sola vez el estado efectivo de conciliación por movimiento.
  ...
  with statuses as materialized (
    ...
    from (
      select source_identifier,source_account,coalesce(source_subcategory,'Sin subcategoría') subcategory,
        ...
        min(coalesce(effective_date,source_date)) first_date,max(coalesce(effective_date,source_date)) last_date,
