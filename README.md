...
- Desde 9.0.0 una factura activa, reciente y con OCR completo puede convertirse en **Factura pendiente** si todavía no está asociada, el matcher canónico no encuentra un movimiento candidato y el calendario no contiene ya un evento equivalente.
- Las facturas pendientes no autoasocian movimientos: únicamente enriquecen el forecast. Cuando aparece el cargo y queda asociado, dejan de generar el compromiso provisional.
- Si el OCR aporta fecha de cargo, pago o vencimiento se usa esa evidencia; en ausencia de fecha explícita se estima factura + 7 días con confianza baja. La ventana documental de admisión es de 45 días.
- Las facturas pendientes usan los mismos descartes reversibles de Previsión y nunca modifican el documento ni el movimiento bancario original.
...
- `forecast_liquidity_core` consume el calendario canónico enriquecido por `forecast_calendar_document_commitments_core`; este a su vez parte de `forecast_calendar_visible_core`. No existe un segundo motor de recurrencias, matching o estimación.
...
- Desde 8.0.0 **Simulador de Decisiones** está disponible en Más → Simulador y permite combinar gastos puntuales, ingresos puntuales, gastos recurrentes y compras a plazos.
- `forecast_scenario_core` reutiliza `forecast_liquidity_core`; superpone únicamente impactos hipotéticos y no introduce un segundo forecast ni recalcula saldos en React. Por ello 9.0 llega al Simulador a través de la misma baseline de liquidez.
- Los escenarios son efímeros: no se almacenan en tablas, `localStorage`, `sessionStorage` ni cookies y no modifican movimientos, cuentas, previsiones, documentos o saldos de origen.
- Cada simulación admite hasta 24 hipótesis y 120 impactos expandidos, y devuelve saldo final, mínimo, fecha del mínimo, días bajo cero, primera fecha de riesgo, hitos 30/60/90, gráfica comparada y calendario hipotético.
...
- Desde 8.0.0 `public.financial_app_forecast_scenario` aplica el mismo patrón y delega en el core privado de escenarios, que vuelve a validar la identidad autorizada antes de calcular.
- Desde 9.0.0 `public.financial_app_forecast_calendar` permanece `SECURITY INVOKER`; el enriquecimiento documental vive en un core privado autorizado y no expone una nueva API pública.
...
- Los gates históricos siguen activos. Desde 6.5.0 las baselines 6.4.0–6.4.11 usan un comparador semántico común; 7.0.0 añade gates específicos de liquidez futura, 8.0.0 del simulador efímero y 9.0.0 de compromisos documentales deduplicados.
- `docs/ARCHITECTURE.md` y `docs/CANONICAL_ARCHITECTURE.md` describen una única arquitectura canónica iniciada en 5.0.0; 9.0.0 amplía esa arquitectura sin crear runtime paralelo.
