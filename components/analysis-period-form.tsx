...
const PERIODS:{value:AnalysisPeriodKey;label:string}[]=[
  {value:"year",label:"Año / año hasta hoy"},
  ...
  {value:"q1",label:"1.er trimestre"},
  {value:"q2",label:"2.º trimestre"},
  {value:"q3",label:"3.er trimestre"},
  {value:"q4",label:"4.º trimestre"},
  {value:"custom",label:"Personalizado"},
];

export function AnalysisPeriodForm({years,year,period,from,to,maxDate}:Props){
  const [selected,setSelected]=useState<AnalysisPeriodKey>(period);
  const custom=selected==="custom";
  return <form className="analysis-period-form" action="/analisis" method="get">
    <label>Periodo
      <select name="period" value={selected} onChange={event=>setSelected(event.target.value as AnalysisPeriodKey)}>
        {PERIODS.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}
      </select>
    </label>
    <label>Año
      <select name="year" defaultValue={String(year)} disabled={selected==="month"||selected==="previous-month"||selected==="last30"||selected==="last3"||selected==="last6"||selected==="last12"}>
        {years.map(value=><option key={value} value={value}>{value}</option>)}
      </select>
    </label>
    <label>Desde
      <input type="date" name="from" defaultValue={from} max={maxDate} disabled={!custom}/>
    </label>
    <label>Hasta
      <input type="date" name="to" defaultValue={to} max={maxDate} disabled={!custom}/>
    </label>
