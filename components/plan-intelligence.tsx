...
export function PlanIntelligence({data}:{data:DecisionIntelligence}){
  return <section className={`plan-intelligence plan-intelligence-${data.posture}`} aria-labelledby="intelligence-title">
    ...
    <div className="plan-intelligence-foot"><span>Motor determinista y explicable. No ejecuta movimientos ni cambia presupuesto, previsiones u objetivos.</span><span>{data.confidenceReasons.length?`Confianza condicionada por: ${data.confidenceReasons.join(" · ")}`:"Sin incidencias de calidad que reduzcan la confianza."}</span><Link href="/inteligencia">Abrir inteligencia accionable →</Link><Link href="/plan/horizonte">Ver horizonte 3/6/12 →</Link></div>
  </section>;
}
