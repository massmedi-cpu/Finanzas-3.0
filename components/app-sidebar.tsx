...
export function AppSidebar({ active, status = "Datos reales · fuente solo lectura" }: { active?: string; status?: string }) {
  const pathname=usePathname();const [moreOpen,setMoreOpen]=useState(false);const currentPath=active||pathname;const desktopNavRef=useRef<HTMLElement|null>(null);
  useEffect(()=>setMoreOpen(false),[pathname]);
  useEffect(()=>{
    const current=desktopNavRef.current?.querySelector<HTMLElement>('a[aria-current="page"]');
    current?.scrollIntoView({block:"nearest",inline:"nearest"});
  },[currentPath]);
  const navLink=(label:string,href:string)=>{const current=matches(currentPath,href);return <IntentLink key={href} className={current?"active":""} href={href} aria-current={current?"page":undefined} onClick={()=>setMoreOpen(false)}>{label}</IntentLink>};
  const moreActive=mobileMore.some(([,href])=>matches(currentPath,href));
  return <aside className="sidebar">
    <a className="skip-link" href="#main-content">Saltar al contenido principal</a>
    <IntentLink className="brand" href="/" aria-label="Financial App · Inicio" onClick={()=>setMoreOpen(false)}><Image className="brand-icon" src="/brand/isotipo.png" width={42} height={42} alt="" priority/><div><strong>Financial App</strong><small>Control financiero personal</small></div></IntentLink>
    <nav ref={desktopNavRef} className="desktop-nav" aria-label="Navegación principal">{sections.map(([label,href])=>navLink(label,href))}</nav>
