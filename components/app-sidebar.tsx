...
export function AppSidebar({ active, status = "Datos reales · fuente solo lectura" }: { active?: string; status?: string }) {
  ...
  const navLink=(label:string,href:string)=>{const current=matches(currentPath,href);return <IntentLink key={href} className={current?"active":""} href={href} aria-current={current?"page":undefined}>{label}</IntentLink>};
  const moreActive=mobileMore.some(([,href])=>matches(currentPath,href));
  return <aside className="sidebar">
    <a className="skip-link" href="#main-content">Saltar al contenido principal</a>
    <IntentLink className="brand" href="/" aria-label="Financial App · Inicio"><Image className="brand-icon" src="/brand/isotipo.png" width={42} height={42} alt="" priority/><div><strong>Financial App</strong><small>Control financiero personal</small></div></IntentLink>
