...
export function ForecastLiquidityDashboard({data,compact=false}:{data:ForecastLiquidityOverview;compact?:boolean}){
  ...
  return <section className={`liquidity-dashboard ${compact?"compact":""}`} aria-labelledby={compact?"liquidity-title-compact":"liquidity-title"}>
    <div className="liquidity-head"><div><p className="eyebrow">AGENDA FINANCIERA · {data.days} DÍAS</p><h2 id={compact?"liquidity-title-compact":"liquidity-title"}>Saldo futuro y próximos compromisos</h2><p>Parte del último saldo de tus cuentas operativas y aplica únicamente movimientos todavía pendientes del mismo motor de previsión.</p></div>{compact&&<Link className="ghost button-link" href="/prevision">Abrir agenda completa</Link>}</div>
