...
test("presupuesto total enlaza al gasto efectivo del mes", async ({ page }) => {
  const snapshot = {
  ...
  await expect(total.getByRole("link", { name: "Ver gastos del mes" })).toHaveAttribute("href", "/transactions?dateFrom=2026-09-01&dateTo=2026-09-30&kind=expense");
});
