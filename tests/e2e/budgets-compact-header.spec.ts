...
test("cabecera de Presupuestos mantiene una jerarquía contenida", async ({ page }) => {
  const snapshot = {
  ...
  const h1 = page.getByRole("heading", { name: "Presupuestos", level: 1 });
  const size = await h1.evaluate((element) => Number.parseFloat(getComputedStyle(element).fontSize));
