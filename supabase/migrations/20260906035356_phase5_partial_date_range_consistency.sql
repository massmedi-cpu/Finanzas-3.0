...
begin
  ...
  if v_date_from is null and v_date_to is null then
    ...
    v_date_from := v_min_date;
    v_date_to := v_max_date;
  elsif v_date_from is null then
    select min(t.bank_date)
      into v_min_date
    from financial_app.transactions t
    where (p_account_id is null or t.account_id=p_account_id)
      and t.bank_date<=v_date_to;
    v_date_from := coalesce(v_min_date,v_date_to);
  elsif v_date_to is null then
    select max(t.bank_date)
      into v_max_date
    from financial_app.transactions t
    where (p_account_id is null or t.account_id=p_account_id)
      and t.bank_date>=v_date_from;
    v_date_to := coalesce(v_max_date,v_date_from);
  end if;
