...
as $$
  select jsonb_build_object(
    ...
    'balances',financial_app.financial_account_balances(p_date_to,p_include_archived,p_account_id),
...
-- The previous two-argument function is no longer canonical. The new third
-- argument has a default, so callers that omit accountId remain compatible.
drop function financial_app.financial_account_balances(date,boolean);
