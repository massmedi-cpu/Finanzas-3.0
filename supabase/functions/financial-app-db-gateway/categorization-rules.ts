...
function nullableUuid(value: unknown, field: string): asserts value is string | null {
  if (value !== null && (typeof value !== "string" || !UUID.test(value))) throw new Error(`invalid_${field}`);
}
...
function nullableSafeInteger(value: unknown, field: string): asserts value is number | null {
  if (value !== null && (typeof value !== "number" || !Number.isSafeInteger(value))) throw new Error(`invalid_${field}`);
}
function priority(value: unknown): asserts value is number {
  if (typeof value !== "number" || !Number.isInteger(value) || value < 0 || value > 1_000_000) throw new Error("invalid_rule_priority");
}
...
function validateRulePayload(payload: any) {
  ...
  if (
    payload.conceptContains === null && payload.merchantId === null && payload.accountId === null &&
    payload.categoryId === null && payload.minimumAmountCents === null && payload.maximumAmountCents === null
  ) throw new Error("rule_condition_required");
  if (payload.targetCategoryId === null && payload.targetMerchantId === null) throw new Error("rule_target_required");
  if (payload.minimumAmountCents !== null && payload.maximumAmountCents !== null && payload.minimumAmountCents > payload.maximumAmountCents) {
...
async function listRules(sql: any) {
  return sql`
    select
      ...
      r.created_at,r.updated_at,cm.name as merchant_name,a.name as account_name,
      cc.name as category_name,tc.name as target_category_name,tm.name as target_merchant_name
...
export async function handleCategorizationRuleAction(input: {
  ...
  if (action === "category.impact") {
    uuid(payload.id, "category_id");
    const rows = await sql`
      select
        (select count(*)::int from financial_app.transactions where category_id=${payload.id}::uuid) as transaction_count,
        (select count(*)::int from financial_app.transaction_overrides where category_override_set=true and category_id_override=${payload.id}::uuid) as override_count,
        (select count(*)::int from financial_app.categorization_rules where status='active' and category_id=${payload.id}::uuid) as rule_condition_count,
        (select count(*)::int from financial_app.categorization_rules where status='active' and target_category_id=${payload.id}::uuid) as rule_target_count,
        (select count(*)::int from financial_app.merchants where lifecycle='active' and default_category_id=${payload.id}::uuid) as merchant_count,
        (select count(*)::int from financial_app.budgets where category_id=${payload.id}::uuid) as budget_count,
        (select count(*)::int from financial_app.recurrences where status='active' and category_id=${payload.id}::uuid) as active_recurrence_count,
        (select count(*)::int from financial_app.forecast_items where excluded=false and confirmed_transaction_id is null and date>=current_date and category_id=${payload.id}::uuid) as future_forecast_count,
        (select count(*)::int from financial_app.categories where lifecycle='active' and parent_category_id=${payload.id}::uuid) as active_child_count
    `;
    const row = rows[0] ?? {};
    return json({
      impact: {
        ...
        ruleConditionCount: row.rule_condition_count ?? 0,
        ruleTargetCount: row.rule_target_count ?? 0,
        merchantCount: row.merchant_count ?? 0,
        budgetCount: row.budget_count ?? 0,
        activeRecurrenceCount: row.active_recurrence_count ?? 0,
        futureForecastCount: row.future_forecast_count ?? 0,
        activeChildCount: row.active_child_count ?? 0,
      },
    });
  }

  if (action === "rule.list") return json({ rows: await listRules(sql) });

  if (action === "rule.save") {
    validateRulePayload(payload);
    const saved = await sql`
      select financial_app.save_categorization_rule(
        ${payload.id ?? null}::uuid,${payload.name},${payload.status},${payload.priority},
        ${payload.conceptContains ?? null},${payload.merchantId ?? null}::uuid,${payload.accountId ?? null}::uuid,
        ${payload.categoryId ?? null}::uuid,${payload.minimumAmountCents ?? null}::bigint,
        ${payload.maximumAmountCents ?? null}::bigint,${payload.targetCategoryId ?? null}::uuid,
  ...
  if (action === "rule.evaluate") {
    ...
    const rows = await sql`select financial_app.evaluate_categorization_rule(${payload.transactionId}::uuid) as result`;
    return json({ result: rows[0]?.result ?? null });
  }
  if (action === "rule.apply") {
    uuid(payload.transactionId, "transaction_id");
    const rows = await sql`select financial_app.apply_categorization_rule(${payload.transactionId}::uuid) as result`;
    return json({ result: rows[0]?.result ?? null });
  }
  if (action === "rule.apply_all") {
    const limit = payload.limit ?? 10000;
    if (typeof limit !== "number" || !Number.isInteger(limit) || limit < 1 || limit > 10000) throw new Error("invalid_rule_apply_limit");
    const rows = await sql`select financial_app.apply_categorization_rules(${limit}::integer) as result`;
    return json({ result: rows[0]?.result ?? null });
  }
  ...
  if (action === "test.categorization_rule_engine") {
    ...
    try {
      await sql.begin(async (tx: any) => {
        const accountRows = await tx`
          ...
          values (${'Rule gateway account ' + token},'Test','checking',0,'EUR','active',0) returning id
        `;
        ...
        const sourceCategoryRows = await tx`
          ...
          values (${'Rule gateway source ' + token},'expense','wallet','category.slate','active',0) returning id
        `;
        const targetCategoryRows = await tx`
          insert into financial_app.categories(name,kind,icon_key,color_token,lifecycle,sort_order)
          values (${'Rule gateway target ' + token},'expense','wallet','category.blue','active',0) returning id
        `;
        ...
        const merchantRows = await tx`
          select financial_app.save_merchant(null,${'Café Regla ' + token},${sourceCategoryId}::uuid,'active') as id
        `;
        const merchantId = merchantRows[0]?.id;
        if (!merchantId) throw new Error("test_rule_merchant_create_failed");
        await tx`select financial_app.save_merchant_alias(null,${merchantId}::uuid,${'TPV CAFÉ REGLA VIP ' + token})`;
        ...
        await tx`select financial_app.save_categorization_rule(null,${'Disabled ' + token},'disabled',0,'cafe',null,null,null,null,null,${sourceCategoryId}::uuid,null)`;
        await tx`select financial_app.save_categorization_rule(null,${'Low ' + token},'active',200,'cafe',null,null,null,null,null,${sourceCategoryId}::uuid,null)`;
        ...
        const evaluationRows = await tx`select financial_app.evaluate_categorization_rule(${transactionId}::uuid) as result`;
        const evaluation = evaluationRows[0]?.result;
        if (
          evaluation?.selectedRuleId !== deterministicRuleId || evaluation?.selectedRulePriority !== 10 ||
          evaluation?.baselineMerchantId !== merchantId || evaluation?.baselineCategoryId !== sourceCategoryId ||
          evaluation?.resolvedCategoryId !== targetCategoryId
        ) throw new Error("test_rule_deterministic_evaluation_failed");
        const applyRows = await tx`select financial_app.apply_categorization_rule(${transactionId}::uuid) as result`;
        const applied = applyRows[0]?.result;
        const storedRows = await tx`select merchant_id,category_id from financial_app.transactions where id=${transactionId}::uuid`;
        if (
          applied?.merchantChanged !== true || applied?.categoryChanged !== true ||
          storedRows[0]?.merchant_id !== merchantId || storedRows[0]?.category_id !== targetCategoryId
        ...
        `;
        const lockedRows = await tx`select financial_app.apply_categorization_rule(${transactionId}::uuid) as result`;
        const locked = lockedRows[0]?.result;
        const afterLockRows = await tx`select category_id from financial_app.transactions where id=${transactionId}::uuid`;
        if (
          locked?.categoryLocked !== true || locked?.categoryChanged !== false ||
          locked?.resolvedCategoryId !== sourceCategoryId || afterLockRows[0]?.category_id !== targetCategoryId
        ) throw new Error("test_rule_manual_override_failed");
        const auditRows = await tx`
          select count(*) filter (where entity_type='rule')::int as rule_changes,
                 count(*) filter (where entity_type='transaction')::int as transaction_changes
          from financial_app.audit_changes where entity_id in (${transactionId}::uuid,${deterministicRuleId}::uuid)
        `;
        if ((auditRows[0]?.rule_changes ?? 0) < 1 || (auditRows[0]?.transaction_changes ?? 0) < 2) throw new Error("test_rule_audit_failed");
        verified = true;
    ...
    const clean = ["accounts", "categories", "merchants", "aliases", "rules", "sources", "transactions", "overrides"].every((key) => residue[key] === 0);
