...
begin
  ...
  insert into financial_app.transaction_overrides(
    ...
    transaction_id,'CORRECCIÓN USUARIO','no perder','confirmed'
  );
  ...
  if result_row.action <> 'skip'
     or result_row.transaction_id <> transaction_id
  ...
  if (select concept_override from financial_app.transaction_overrides where transaction_id=transaction_id) <> 'CORRECCIÓN USUARIO' then
    raise exception 'override_lost';
  end if;
  if (select note from financial_app.transaction_overrides where transaction_id=transaction_id) <> 'no perder' then
    raise exception 'override_note_lost';
  end if;
  if not exists(
    select 1 from financial_app.audit_changes
    where entity_id=transaction_id and field_name='source_revision' and change_origin='source_sync'
  ) then
  ...
  if (select duplicate_state from financial_app.transactions where id=transaction_id) <> 'suspected' then
